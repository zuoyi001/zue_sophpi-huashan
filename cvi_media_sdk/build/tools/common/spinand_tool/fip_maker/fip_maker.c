...
void main(int argc, char const *argv[])
{
	...
	unsigned int pg_sz, pg_per_blk, bk_sz;
	unsigned int total_len, ttl_block_cnt_to_write, ttl_pg_cnt_to_write;
	...
	if (argc < 2) {
		printf("Use default NAND setting\n");
		argv[1] = "0xFFFF";
	}

	id = strtol(argv[1], NULL, 0);

	printf("get NAND ID: 0x%x\n", id);
	...
	case 0x22aaef:
		...
		nand.fip_block_cnt = 20;
		nand.pages_per_block_shift = 6;
		nand.flags = FLAGS_ENABLE_X2_BIT | FLAGS_ENABLE_X4_BIT;
		nand.ecc_en_feature_offset = 0xB0;
		nand.ecc_en_mask = 1 << 4;
		nand.ecc_status_offset = 0xC0;
		nand.ecc_status_mask = 0x30;
	...
	case 0x90c2:
		...
		nand.fip_block_cnt = 20;
		nand.pages_per_block_shift = 6;
		nand.flags = FLAGS_SET_PLANE_BIT | FLAGS_SET_QE_BIT | FLAGS_ENABLE_X2_BIT | FLAGS_ENABLE_X4_BIT;
		nand.ecc_en_feature_offset = 0xB0;
		nand.ecc_en_mask = 1 << 4;
		nand.ecc_status_offset = 0xC0;
		nand.ecc_status_mask = 0x30;
	...
	case 0xFFFF:
		...
		break;
	default:
		printf("Input id 0x%x is not in support list\n", id);
		return;
	}
	//nand.dummy_last[0] = 0xFF;

	stat("./fip.bin", &statbuf);
	...
	inf = fopen("./fip.bin", "rb");
	if (!inf) {
		printf("read fip.bin failed\n");
		return;
	}

	fread(buff, sizeof(unsigned char), size, inf);

	outf = fopen("./fip_out.bin", "wb");

	pg_sz = nand.page_size;
	bk_sz = nand.block_size;
	...
	printf("Re-Assemble fip.bin to fip_mar.bin\n");
	...
	fwrite(out_buff, sizeof(unsigned char), total_len, outf);
	printf("fip.bin --> fip_mars.bin  is completed!!\n");
