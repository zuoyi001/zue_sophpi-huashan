...
static void sdhci_phobos_emmc_setup_pad(struct sdhci_host *host)
{
...
static void sdhci_phobos_sd_setup_pad(struct sdhci_host *host, bool bunplug)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
static void sdhci_phobos_sd_setup_io(struct sdhci_host *host, bool reset)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
static int sdhci_phobos_general_select_drive_strength(struct sdhci_host *host,
...
static void sdhci_cvi_phobos_set_tap(struct sdhci_host *host, unsigned int tap)
{
	pr_debug("%s %d\n", __func__, tap);
	// Set sd_clk_en(0x2c[2]) to 0
	sdhci_writew(host, sdhci_readw(host, SDHCI_CLOCK_CONTROL) & (~(0x1 << 2)), SDHCI_CLOCK_CONTROL);
	sdhci_writel(host,
		sdhci_readl(host, CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R) & (~(BIT(1))),
		CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R);
	sdhci_writel(host, BIT(8) | tap << 16,
		     CVI_PHOBOS_SDHCI_PHY_TX_RX_DLY);
	sdhci_writel(host, 0, CVI_PHOBOS_SDHCI_PHY_CONFIG);
	// Set sd_clk_en(0x2c[2]) to 1
	sdhci_writew(host, sdhci_readw(host, SDHCI_CLOCK_CONTROL) | (0x1 << 2), SDHCI_CLOCK_CONTROL);
	mdelay(1);
}

static int sdhci_phobos_general_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	u16 min = 0;
	u32 k = 0;
	s32 ret;
	...
	while (min < TUNE_MAX_PHCODE) {
		retry_cnt = 0;
		sdhci_cvi_phobos_set_tap(host, min);
		reg_rx_lead_lag = sdhci_readw(host, CVI_PHOBOS_SDHCI_PHY_DLY_STS) & BIT(1);
...
retry_tuning:
	...
	sdhci_cvi_phobos_set_tap(host, final_tap);
...
static void sdhci_phobos_emmc_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
	...
	sdhci_writel(host,
			 sdhci_readl(host, CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R) | BIT(0),
			 CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R);
	...
	if (ctrl_2 == SDHCI_CTRL_UHS_SDR104) {
		...
		//reg_0x24c[0] = 0
		sdhci_writel(host,
			sdhci_readl(host, CVI_PHOBOS_SDHCI_PHY_CONFIG) & ~(BIT(0)),
			CVI_PHOBOS_SDHCI_PHY_CONFIG);
		...
		sdhci_writel(host, 0x1000100, CVI_PHOBOS_SDHCI_PHY_TX_RX_DLY);
	}
}

static void sdhci_phobos_sd_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
static void sdhci_phobos_sdio_reset(struct sdhci_host *host, u8 mask)
{
	u16 ctrl_2;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
	...
	if (ctrl_2 == SDHCI_CTRL_UHS_SDR104) {
		...
		sdhci_writel(host,
			sdhci_readl(host, CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R) | BIT(16),
			CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R);
		//reg_0x24c[0] = 0
		sdhci_writel(host,
			sdhci_readl(host, CVI_PHOBOS_SDHCI_PHY_CONFIG) & ~(BIT(0)),
...
void sdhci_phobos_sd_voltage_switch(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
...
void sdhci_phobos_sd_voltage_restore(struct sdhci_host *host, bool bunplug)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
	...
	sdhci_writel(host,
		sdhci_readl(host, CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R) | BIT(1) | BIT(8) | BIT(9),
		CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R);
	sdhci_writel(host, 0x1000100, CVI_PHOBOS_SDHCI_PHY_TX_RX_DLY);
	sdhci_writel(host, 1, CVI_PHOBOS_SDHCI_PHY_CONFIG);

	mdelay(1);
}

static void sdhci_phobos_sd_set_power(struct sdhci_host *host, unsigned char mode,
				unsigned short vdd)
{
	struct mmc_host *mmc = host->mmc;
	...
	if (mode == MMC_POWER_ON && mmc->ops->get_cd(mmc)) {
		...
		sdhci_phobos_sd_voltage_restore(host, false);
		sdhci_phobos_sd_setup_pad(host, false);
		sdhci_phobos_sd_setup_io(host, false);
		mdelay(5);
	} else if (mode == MMC_POWER_OFF) {
		sdhci_phobos_sd_setup_pad(host, true);
		sdhci_phobos_sd_setup_io(host, true);
		sdhci_phobos_sd_voltage_restore(host, true);
		sdhci_set_power_noreg(host, mode, vdd);
		mdelay(30);
	}
}

static void sdhci_phobos_emmc_dump_vendor_regs(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
	...
	SDHCI_DUMP(": Reg_200:   0x%08x | Reg_240:  0x%08x\n",
		   sdhci_readl(host, CVI_PHOBOS_SDHCI_VENDOR_MSHC_CTRL_R),
		   sdhci_readl(host, CVI_PHOBOS_SDHCI_PHY_TX_RX_DLY));
	SDHCI_DUMP(": Reg_244:   0x%08x | Reg_248:  0x%08x\n",
		   sdhci_readl(host, CVI_PHOBOS_SDHCI_PHY_DS_DLY),
		   sdhci_readw(host, CVI_PHOBOS_SDHCI_PHY_DLY_STS));
	SDHCI_DUMP(": Reg_24C:   0x%08x\n",
		   sdhci_readl(host, CVI_PHOBOS_SDHCI_PHY_CONFIG));
...
static void sdhci_phobos_sd_dump_vendor_regs(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_cvi_host *cvi_host = sdhci_pltfm_priv(pltfm_host);
	u8 clk_source_select = 0;
...
static const struct sdhci_ops sdhci_phobos_emmc_ops = {
	.reset = sdhci_phobos_emmc_reset,
	...
	.platform_execute_tuning = sdhci_phobos_general_execute_tuning,
	.select_drive_strength = sdhci_phobos_general_select_drive_strength,
	.dump_vendor_regs = sdhci_phobos_emmc_dump_vendor_regs,
};

static const struct sdhci_ops sdhci_phobos_sd_ops = {
	.reset = sdhci_phobos_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_power = sdhci_phobos_sd_set_power,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
	.voltage_switch = sdhci_phobos_sd_voltage_switch,
	.set_uhs_signaling = sdhci_cvi_general_set_uhs_signaling,
	.platform_execute_tuning = sdhci_phobos_general_execute_tuning,
	.select_drive_strength = sdhci_phobos_general_select_drive_strength,
	.dump_vendor_regs = sdhci_phobos_sd_dump_vendor_regs,
};

static const struct sdhci_ops sdhci_phobos_sdio_ops = {
	.reset = sdhci_phobos_sdio_reset,
	.set_clock = sdhci_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
	.voltage_switch = sdhci_phobos_sd_voltage_switch,
...
static const struct sdhci_ops sdhci_phobos_fpga_emmc_ops = {
	.reset = sdhci_phobos_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_bus_width = sdhci_set_bus_width,
	.get_max_clock = sdhci_cvi_general_get_max_clock,
...
static const struct sdhci_ops sdhci_phobos_fpga_sd_ops = {
	.reset = sdhci_phobos_sd_reset,
	.set_clock = sdhci_set_clock,
	.set_power = sdhci_phobos_sd_set_power,
	.set_bus_width = sdhci_set_bus_width,
...
static const struct sdhci_pltfm_data sdhci_phobos_emmc_pdata = {
	.ops = &sdhci_phobos_emmc_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
};

static const struct sdhci_pltfm_data sdhci_phobos_sd_pdata = {
	.ops = &sdhci_phobos_sd_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
};

static const struct sdhci_pltfm_data sdhci_phobos_sdio_pdata = {
	.ops = &sdhci_phobos_sdio_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_NO_1_8_V,
};

static const struct sdhci_pltfm_data sdhci_phobos_fpga_emmc_pdata = {
	.ops = &sdhci_phobos_fpga_emmc_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_BROKEN_HS200,
};

static const struct sdhci_pltfm_data sdhci_phobos_fpga_sd_pdata = {
	.ops = &sdhci_phobos_fpga_sd_ops,
	.quirks = SDHCI_QUIRK_INVERTED_WRITE_PROTECT | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_NO_1_8_V,
};

static const struct of_device_id sdhci_cvi_dt_match[] = {
	{.compatible = "cvitek,phobos-fpga-emmc", .data = &sdhci_phobos_fpga_emmc_pdata},
	{.compatible = "cvitek,phobos-fpga-sd", .data = &sdhci_phobos_fpga_sd_pdata},
	{.compatible = "cvitek,phobos-emmc", .data = &sdhci_phobos_emmc_pdata},
	{.compatible = "cvitek,phobos-sd", .data = &sdhci_phobos_sd_pdata},
	{.compatible = "cvitek,phobos-sdio", .data = &sdhci_phobos_sdio_pdata},
...
static int sdhci_cvi_probe(struct platform_device *pdev)
{
	...
	if (strstr(dev_name(mmc_dev(host->mmc)), "cv-emmc"))
		sdhci_phobos_emmc_setup_pad(host);
...
#ifdef CONFIG_ARCH_CVPHOBOS_ASIC
