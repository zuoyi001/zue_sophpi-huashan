...
static int cvsnfc_ecc_probe(struct cvsnfc_host *host)
{
	...
	mtd->bitflip_threshold = info->ecc.strength_ds;
...
static int parse_status_info(struct cvsnfc_host *host)
{
	...
	if (ecc_info->ecc_sr_addr && ecc_info->read_ecc_opcode) {
		pr_err("error ecc information,please check it!\n");
		return -1;
	}

	if (ecc_info->ecc_sr_addr)
		spi_feature_op(host, GET_OP, ecc_info->ecc_sr_addr, &ecc_status);

	if (ecc_info->read_ecc_opcode)
		spi_nand_read_eccsr(host, ecc_info->read_ecc_opcode, &ecc_status);

	mask = GENMASK(ecc_info->ecc_bits - 1, 0);
	status = (ecc_status >> ecc_info->ecc_bit_shift) & mask;

	pr_debug("ecc status:%#x, mask:%#x\n", ecc_status, mask);
	...
	if (status > ecc_info->uncorr_val && ecc_info->remap)
		return ecc_info->remap[status] != 0xff ? ecc_info->remap[status] : 0;
