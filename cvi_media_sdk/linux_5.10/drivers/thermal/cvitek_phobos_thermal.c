...
struct phobos_thermal_zone {
	unsigned int ch;
	void __iomem *base;
	struct phobos_thermal *ct;
};

struct phobos_thermal {
	struct device *dev;
	void __iomem *base;
	struct clk *clk_tempsen;
};

static void phobos_thermal_init(struct phobos_thermal *ct)
{
...
static void phobos_thermal_uninit(struct phobos_thermal *ct)
{
	void __iomem *base = ct->base;
	u32 regval;
...
static int phobos_read_temp(void *data, int *temperature)
{
	struct phobos_thermal_zone *ctz = data;
...
static const struct thermal_zone_of_device_ops phobos_thermal_ops = {
	.get_temp = phobos_read_temp,
};

static const struct of_device_id phobos_thermal_of_match[] = {
	{
		.compatible = "cvitek,phobos-thermal",
	},
	{},
};
MODULE_DEVICE_TABLE(of, phobos_thermal_of_match);

static int phobos_thermal_probe(struct platform_device *pdev)
{
	struct phobos_thermal *ct;
	struct phobos_thermal_zone *ctz;
	struct resource *res;
...
static int phobos_thermal_remove(struct platform_device *pdev)
{
	struct phobos_thermal *ct = platform_get_drvdata(pdev);

	phobos_thermal_uninit(ct);
...
static int phobos_thermal_suspend(struct device *dev)
{
	struct phobos_thermal *ct = dev_get_drvdata(dev);
...
static int phobos_thermal_resume(struct device *dev)
{
	struct phobos_thermal *ct = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(phobos_thermal_pm_ops,
			 phobos_thermal_suspend, phobos_thermal_resume);

static struct platform_driver phobos_thermal_driver = {
	.probe = phobos_thermal_probe,
	.remove = phobos_thermal_remove,
	.driver = {
		.name = "phobos-thermal",
		.pm = &phobos_thermal_pm_ops,
		.of_match_table = phobos_thermal_of_match,
	},
};

module_platform_driver(phobos_thermal_driver);
MODULE_DESCRIPTION("Mars thermal driver");
