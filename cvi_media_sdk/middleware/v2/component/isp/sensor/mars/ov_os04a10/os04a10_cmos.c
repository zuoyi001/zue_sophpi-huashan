...
static CVI_S32 cmos_again_calc_table(VI_PIPE ViPipe, CVI_U32 *pu32AgainLin, CVI_U32 *pu32AgainDb)
{
	...
	if (*pu32AgainLin >= (int)(Again_table[63] * rate)) {
		*pu32AgainLin = (int)(Again_table[63] * rate);
		*pu32AgainDb = 63;
		return CVI_SUCCESS;
	}

	for (i = 1; i < 64; i++) {
		if (*pu32AgainLin < (int)(Again_table[i] * rate)) {
			*pu32AgainLin = (int)(Again_table[i - 1] * rate);
			*pu32AgainDb = i - 1;
			break;
		}
	}
