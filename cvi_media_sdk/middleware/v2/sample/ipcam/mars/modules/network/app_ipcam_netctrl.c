...
static int GetAiInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    cJSON_AddNumberToObject(cjsonAiAttr, "md_enable", pstMdInfo->bEnable);
    cJSON_AddNumberToObject(cjsonAiAttr, "md_threshold", pstMdInfo->threshold);

    // pd

    cJSON_AddNumberToObject(cjsonAiAttr, "pd_enable", pstPdInfo->bEnable);
...
static int GetOsdInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    printf("\n");
    // timestamp
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_time", pstOsdInfo->astOsdAttr[0].bShow);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_ts_x", pstOsdInfo->astOsdAttr[0].stRect.s32X);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_ts_y", pstOsdInfo->astOsdAttr[0].stRect.s32Y);
    // text1
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text1", pstOsdInfo->astOsdAttr[1].bShow);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text1_x", pstOsdInfo->astOsdAttr[1].stRect.s32X);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text1_y", pstOsdInfo->astOsdAttr[1].stRect.s32Y);
    cJSON_AddStringToObject(cjsonOsdAttr, "osd_text1_content", pstOsdInfo->astOsdAttr[1].str);
    // text2
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text2", pstOsdInfo->astOsdAttr[7].bShow);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text2_x", pstOsdInfo->astOsdAttr[7].stRect.s32X);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text2_y", pstOsdInfo->astOsdAttr[7].stRect.s32Y);
    cJSON_AddStringToObject(cjsonOsdAttr, "osd_text2_content", pstOsdInfo->astOsdAttr[7].str);
    // text3
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text3", pstOsdInfo->astOsdAttr[4].bShow);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text3_x", pstOsdInfo->astOsdAttr[4].stRect.s32X);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_text3_y", pstOsdInfo->astOsdAttr[4].stRect.s32Y);
    cJSON_AddStringToObject(cjsonOsdAttr, "osd_text3_content1", pstOsdInfo->astOsdAttr[4].str);
    cJSON_AddStringToObject(cjsonOsdAttr, "osd_text3_content2", pstOsdInfo->astOsdAttr[5].str);
    cJSON_AddStringToObject(cjsonOsdAttr, "osd_text3_content3", pstOsdInfo->astOsdAttr[6].str);
    // privacy area
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_privacy", pstOsdcPrivacy->bShow);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_privacy_x", pstOsdcPrivacy->x1);
    cJSON_AddNumberToObject(cjsonOsdAttr, "osd_privacy_y", pstOsdcPrivacy->y1);
...
static int app_ipcam_OsdString_Update(APP_PARAM_OSD_ATTR_S *pstOsdAttr, RGN_CHN_ATTR_S *regChnAttr)
{
    ...
    strLen = strnlen(pstOsdAttr->str, 128);
    str = pstOsdAttr->str;
    printf("osd handle:%d SingleText str:%s Len:%d\n", pstOsdAttr->Handle, str, strLen);

    ret = CVI_RGN_DetachFromChn(pstOsdAttr->Handle, &pstOsdAttr->stChn);
    if (ret != CVI_SUCCESS) {
        printf("CVI_RGN_DetachFromChn failed with %#x!\n", ret);
    }
    ret = CVI_RGN_Destroy(pstOsdAttr->Handle);
    if (ret != CVI_SUCCESS) {
        printf("CVI_RGN_Destroy failed with %#x!\n", ret);
    }
    regAttr.enType = OVERLAY_RGN;
    regAttr.unAttr.stOverlay.enPixelFormat = PIXEL_FORMAT_ARGB_1555;
    regAttr.unAttr.stOverlay.u32BgColor = 0x00000000; // ARGB1555 transparent
    regAttr.unAttr.stOverlay.u32CanvasNum = 1;
    regAttr.unAttr.stOverlay.stSize.u32Height = 24;
    regAttr.unAttr.stOverlay.stSize.u32Width = 24 * strLen;
    regAttr.unAttr.stOverlay.stCompressInfo.enOSDCompressMode = OSD_COMPRESS_MODE_NONE; // MARS only

    ret = CVI_RGN_Create(pstOsdAttr->Handle, &regAttr);
    if (ret != CVI_SUCCESS) {
        printf("CVI_RGN_Create failed with %#x!\n", ret);
        return CVI_FAILURE;
    }
    ret = CVI_RGN_AttachToChn(pstOsdAttr->Handle, &pstOsdAttr->stChn, regChnAttr);
    if (ret != CVI_SUCCESS) {
        printf("CVI_RGN_AttachToChn failed with %#x!\n", ret);
        return ret;
    }

    ret = CVI_RGN_SetAttr(pstOsdAttr->Handle, &regAttr);
    if (ret != CVI_SUCCESS) {
        printf("CVI_RGN_GetAttr failed \n");
        return ret;
    }

    bitmap.u32Width = regAttr.unAttr.stOverlay.stSize.u32Width;
    bitmap.u32Height = regAttr.unAttr.stOverlay.stSize.u32Height;
    bitmap.enPixelFormat = regAttr.unAttr.stOverlay.enPixelFormat;
    bitmap.pData = malloc(2 * bitmap.u32Width * bitmap.u32Height);
    if (bitmap.pData == NULL) {
        printf("malloc osd memroy err!\n");
        return ret;
    }

    ret = app_ipcam_Osd_Bitmap_Update(str, &bitmap);
    if (ret != CVI_SUCCESS) {
        printf("UpdateBitmap failed!\n");
        free(bitmap.pData);
        return ret;
    }
    ret = CVI_RGN_SetBitMap(pstOsdAttr->Handle, &bitmap);
    if (ret != CVI_SUCCESS) {
...
static int app_ipcam_OsdInfo_Set(APP_PARAM_OSD_CFG_T *pstOsdInfo)
{
    CVI_U32 osdCnt, ret;
    CVI_U32 i;
    printf("enter: %s\n", __func__);

    osdCnt = pstOsdInfo->osd_cnt;
    printf("osdCnt: %d bEnable:%d\n", osdCnt, pstOsdInfo->bEnable);
    
    for (i = 0; i < osdCnt; i++) {
        RGN_HANDLE handle = pstOsdInfo->astOsdAttr[i].Handle;
        MMF_CHN_S *pstMmfChn = &pstOsdInfo->astOsdAttr[i].stChn;
        RGN_CHN_ATTR_S regChnAttr = {0};

        ret = CVI_RGN_GetDisplayAttr(handle, pstMmfChn, &regChnAttr);
        if (ret != CVI_SUCCESS) {
            printf("CVI_RGN_GetDisplayAttr failed \n");
        }
        ...
        if ((pstOsdInfo->astOsdAttr[i].enType == TYPE_STRING) && pstOsdInfo->astOsdAttr[i].bChange) {
            app_ipcam_OsdString_Update(&pstOsdInfo->astOsdAttr[i], &regChnAttr);
        }
    }
