...
static APP_PARAM_AI_FD_CFG_S stFdCfg = {
    .FD_bEnable = CVI_TRUE,
    .FR_bEnable = CVI_FALSE,
    .MASK_bEnable = CVI_FALSE,
    .CAPURE_bEnable = CVI_TRUE,
    .FACE_AE_bEnable = CVI_TRUE,
    .VpssGrp = 5,
    .VpssChn = 0,
    .FdPoolId = 6,
    .u32GrpWidth = 1280,
    ...
    .model_size_w = 608,
    .model_size_h = 342,
    .bVpssPreProcSkip = CVI_FALSE,
    .threshold_fd = 0.7,
    .threshold_fr = 0.7,
    .threshold_mask = 0.7,
    .model_id_fd = CVI_AI_SUPPORTED_MODEL_RETINAFACE,
    .model_id_fr = CVI_AI_SUPPORTED_MODEL_FACERECOGNITION,
    .model_id_mask = CVI_AI_SUPPORTED_MODEL_MASKCLASSIFICATION,
    .model_path_fd = "/mnt/data/ai_models/retinaface_mnet0.25_342_608.cvimodel",
...
static CVI_S32 app_ipcam_Ai_FR_Init(void)
{
   /* face_name list */
    ...
    ret = CVI_AI_SetModelPath(Ai_Fd_Handle, pstFdCfg->model_id_fr, pstFdCfg->model_path_fr);
    if (ret != CVI_SUCCESS)
    {
...
static  CVI_S32 app_ipcam_Face_Registeration(CVI_VOID)
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        { 
            ...
            ret = CVI_AI_FaceMaskDetection(Ai_Fd_Handle, &fdFrame, &face_test);
            if (ret != CVI_SUCCESS)
            {
                APP_PROF_LOG_PRINT(LEVEL_ERROR, "%s CVI_AI_FaceMaskDetection fail\n",ptr->d_name);
                goto REGISTER_EXIT;
            }
...
static CVI_S32 app_ipcam_Ai_Fd_Proc_Init(CVI_VOID)
{
    ...
    //retinaface
    s32Ret = CVI_AI_SetModelPath(Ai_Fd_Handle, pstFdCfg->model_id_fd, pstFdCfg->model_path_fd);
    if (s32Ret != CVI_SUCCESS)
    {
    ...
    s32Ret = CVI_AI_SetVpssTimeout(Ai_Fd_Handle, 4000);
    if (s32Ret != CVI_SUCCESS)
    {
    ...
    if (pstFdCfg->CAPURE_bEnable)
    {
        s32Ret = app_ipcam_Ai_Face_Capure_Init(&Ai_Fd_Handle);
        if (s32Ret != CVI_SUCCESS)
        {
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "Face Capure init failed with %#x!\n", s32Ret);
        }
    }

     /* 5.Attach Vb POOL For FD Function */
...
static CVI_VOID *Thread_FD_PROC(CVI_VOID *arg)
{
    ...
    while (app_ipcam_Ai_FD_ProcStatus_Get()) 
    {
        ...
        if (pstFdCfg->CAPURE_bEnable)
        {
            s32Ret=app_ipcam_Ai_Face_Capure(&stfdFrame,&face);
            if (s32Ret != CVI_SUCCESS)
            {
                APP_PROF_LOG_PRINT(LEVEL_ERROR,"app_ipcam_Ai_Face_Capure fail \n");
                goto FD_FAILURE;
            }
        }
        else
        {
            s32Ret = CVI_AI_FaceMaskDetection(Ai_Fd_Handle, &stfdFrame, &face);
            if (s32Ret != CVI_SUCCESS)
            {
                APP_PROF_LOG_PRINT(LEVEL_ERROR,"CVI_AI_FaceMaskDetection fail \n");
                goto FD_FAILURE;
            }
        }
     ...
     FD_FAILURE:
        ...
        stFdjDraw.size = face.size;
        stFdjDraw.rescale_type = RESCALE_RB;
        stFdjDraw.width = face.width;
        stFdjDraw.height = face.height;
...
int app_ipcam_Ai_FD_Start(void)
{
    ...
    if (s32Ret != CVI_SUCCESS)
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "AI app_ipcam_Ai_Fd_Proc_Init failed!\n");
        return s32Ret;
    }
