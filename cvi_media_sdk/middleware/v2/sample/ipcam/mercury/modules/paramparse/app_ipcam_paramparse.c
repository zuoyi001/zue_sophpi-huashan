...
static int _Load_Param_Sys(const char *file, APP_PARAM_SYS_CFG_S *Sys)
{
    ...
    for (i = 0; i < Sys->vb_pool_num; i++) {
        ...
        Sys->vb_pool[i].width      = ini_getl(tmp_section, "frame_width", 0, file);
        Sys->vb_pool[i].height     = ini_getl(tmp_section, "frame_height", 0, file);
        Sys->vb_pool[i].fmt        = ini_getl(tmp_section, "frame_fmt", 0, file);
        Sys->vb_pool[i].enBitWidth = ini_getl(tmp_section, "data_bitwidth", 0, file);
        Sys->vb_pool[i].enCmpMode  = ini_getl(tmp_section, "compress_mode", 0, file);
        Sys->vb_pool[i].vb_blk_num = ini_getl(tmp_section, "blk_cnt", 0, file);

        APP_PROF_LOG_PRINT(LEVEL_INFO, "vb_pool[%d] w=%4d h=%4d count=%d fmt=%d\n", i, 
            Sys->vb_pool[i].width, Sys->vb_pool[i].height, Sys->vb_pool[i].vb_blk_num, Sys->vb_pool[i].fmt);
    }
...
static int _Load_Param_Venc(const char *file, APP_PARAM_VENC_CTX_S *Venc)
{
    ...
    for (i = 0; i < chn_num; i++) {
        ...
        if (Venc->astVencChnCfg[i].enType != PT_JPEG) {
            ...
            Venc->astVencChnCfg[i].fr32DstFrameRate = ini_getl(tmp_section, "dst_framerate", 0, file);
...
static int _Load_Param_Audio(const char *file, APP_PARAM_AUDIO_CFG_T *Auido)
{
    ...
    Auido->astAudioVqe.mAiAgcCfg.para_agc_vad_enable  = ini_getl("audio_vqe_agc", "ai_vad_enable", 0, file);
    Auido->astAudioVqe.mAiAgcCfg.para_agc_vad_cnt     = ini_getl("audio_vqe_agc", "ai_vad_cnt", 0, file);
    Auido->astAudioVqe.mAiAgcCfg.para_agc_cut6_enable = ini_getl("audio_vqe_agc", "ai_cut6_enable", 0, file);
    ...
    Auido->astAudioVqe.mAiAnrCfg.para_nr_noise_coeff  = ini_getl("audio_vqe_anr", "ai_noise_coeff", 0, file);
    ...
    Auido->astAudioVqe.mAoAgcCfg.para_agc_vad_enable  = ini_getl("audio_vqe_agc", "ao_vad_enable", 0, file);
    Auido->astAudioVqe.mAoAgcCfg.para_agc_vad_cnt     = ini_getl("audio_vqe_agc", "ao_vad_cnt", 0, file);
    Auido->astAudioVqe.mAoAgcCfg.para_agc_cut6_enable = ini_getl("audio_vqe_agc", "ao_cut6_enable", 0, file);
    ...
    Auido->astAudioVqe.mAoAnrCfg.para_nr_noise_coeff  = ini_getl("audio_vqe_anr", "ao_noise_coeff", 0, file);
...
static int _Load_Param_Osd(const char *file, APP_PARAM_OSD_CFG_T *Osd)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_INFO, "osd_cnt: %d\n", Osd->osd_cnt);
    ...
    for (i = 0; i < Osd->osd_cnt; i++) {
        ...
        if (Osd->astOsdAttr[i].enType == TYPE_STRING) {
            ini_gets(tmp_section, "str", " ", tmp_buff, PARAM_STRING_LEN, file);
            strncpy(Osd->astOsdAttr[i].str, tmp_buff, PARAM_STRING_LEN);
