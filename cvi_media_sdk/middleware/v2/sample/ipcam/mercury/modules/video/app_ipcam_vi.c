 ...
 **************************************************************************/
#ifdef SUPPORT_ISP_PQTOOL
#define IQ_PARAM_BIN        "/mnt/data/cvi_sdr_bin"
#else
#define IQ_PARAM_BIN        "/mnt/cfg/param/cvi_sdr_bin"
#endif
...
static CVI_VOID app_ipcam_Ispd_Load(CVI_VOID)
{
    ...
    if (!bISPDaemon) {
        ...
        if (pISPDHandle) {
            ...
            daemon_init = dlsym(pISPDHandle, "isp_daemon_init");
...
int app_ipcam_Vi_Sensor_Start(void)
{
    ...
    for (CVI_U32 i = 0; i < g_pstViCtx->u32WorkSnsCnt; i++) {
        ...
        isp_cmos_sensor_image_mode.f32Fps    = pstChnCfg->f32Fps;
...
static CVI_S32 app_ipcam_PQBin_Load(void)
{
    ...
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_INFO, "GetBinName(%s) fail\n", binName);
        return ret;
    }
...
int app_ipcam_Vi_Isp_Init(void)
{
    ...
    for (CVI_U32 i = 0; i < g_pstViCtx->u32WorkSnsCnt; i++) {
        ...
        s32Ret = CVI_BIN_SetBinName(stPubAttr.enWDRMode, IQ_PARAM_BIN);
        if (s32Ret != CVI_SUCCESS) {
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_BIN_SetBinName %s failed with %#x!\n", IQ_PARAM_BIN, s32Ret);
            return s32Ret;
        }

        s32Ret = app_ipcam_PQBin_Load();
        if (s32Ret != CVI_SUCCESS) {
...
int app_ipcam_Vi_Isp_Start(void)
{
    ...
    for (CVI_U32 i = 0; i < g_pstViCtx->u32WorkSnsCnt; i++) {
        ...
        s32Ret = CVI_BIN_SetBinName(pstDevAttr.stWDRAttr.enWDRMode, IQ_PARAM_BIN);
        if (s32Ret != CVI_SUCCESS) {
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_BIN_SetBinName %s failed with %#x!\n", IQ_PARAM_BIN, s32Ret);
            return s32Ret;
        }
...
#ifdef ARCH_CV182X
#define IRUT_GPIO_A CVI_GPIOC_16
#define IRUT_GPIO_B CVI_GPIOC_17
#define WHITE_LED   -1
#define IR_LED      -1
#endif

#ifdef ARCH_CV183X
#define IRUT_GPIO_A CVI_GPIOB_03
#define IRUT_GPIO_B CVI_GPIOB_05
#define WHITE_LED   -1
#define IR_LED      -1
#endif
...
static CVI_VOID *Thread_Rgb_Ir_Proc(CVI_VOID *pArgs)
{
	...
	while(Auto_Rgb_Ir_Enable)
	{
			...
			if(stIrAttr.enIrSwitch == 2)
			{
                APP_PROF_LOG_PRINT(LEVEL_INFO, "switch to ir modle\n");

				CVI_BIN_SetBinName(WDR_MODE_NONE, "/mnt/data/cvi_sdr_ir_bin");
				app_ipcam_PQBin_Load();
				...
				//打开red led
                app_ipcam_Speaker_Enable(IR_LED, 1);
				stIrAttr.enIrStatus = 1;
			}
			else if(stIrAttr.enIrSwitch == 1)
			{
				...
				//关闭红外
				app_ipcam_Speaker_Enable(IR_LED, 0);
				stIrAttr.enIrStatus = 0;

				CVI_BIN_SetBinName(WDR_MODE_NONE, "/mnt/data/cvi_sdr_bin");
				app_ipcam_PQBin_Load();
				#ifdef USING_VPSS_ADJUSTMENT
				CVI_VPSS_SetGrpParamfromBin(0, 0);//grop0 load scene0
				#endif
            ...
            case 's': 
                {
                    ...
                    if (modle == 1) {//Infrared night vision mode case
                        ...
                        app_ipcam_Speaker_Enable(IR_LED, 1);
                        //white led close
                        app_ipcam_Speaker_Enable(WHITE_LED, 1);
                        ...
                        //ir/white led close
                        app_ipcam_Speaker_Enable(IR_LED, 0);
                        app_ipcam_Speaker_Enable(WHITE_LED, 0);
                        ...
                        CVI_BIN_SetBinName(WDR_MODE_NONE, "/mnt/data/cvi_night_color_bin");
                        app_ipcam_PQBin_Load();
                        #ifdef USING_VPSS_ADJUSTMENT
                        CVI_VPSS_SetGrpParamfromBin(0, 0);//grop0 load scene0
                        #endif
                        ...
                        CVI_BIN_SetBinName(WDR_MODE_NONE, "/mnt/data/cvi_wdr_bin");
                        app_ipcam_PQBin_Load();
                        #ifdef USING_VPSS_ADJUSTMENT
                        CVI_VPSS_SetGrpParamfromBin(0, 0);//grop0 load scene0
                        #endif
