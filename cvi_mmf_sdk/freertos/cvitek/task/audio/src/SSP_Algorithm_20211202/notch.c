...
void notch_filter(short *pin, short *pout, qfilter_struct *eq, int frame_size)
{
  long long acc0;
	...
	for(i = frame_size; i > 0; i--) {
		...
		for(j = eq->nq; j > 0; j--) {
			...
			*pst++ = yn1;  /* update y(n-2) */
			...
			acc0 = MAX(((int)0x80000000), (MIN(0x7fffffff, (((int)acc0)<<2))));  /* b,a coefs are Q2.14 and the final coef gain is Q3.13 */
		...
		acc0 = MAX(((int)0x80000000), (MIN(0x7fffffff, ((acc0)<<3))));  /* b,a coefs are Q2.14 and the final coef gain is Q3.13 */
		*ptr2++ = (short)(acc0>>16);
