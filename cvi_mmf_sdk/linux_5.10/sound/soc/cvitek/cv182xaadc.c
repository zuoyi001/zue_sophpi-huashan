...
static DEFINE_MUTEX(cv182xaadc_mutex);
...
{
	if (mutex_lock_interruptible(&cv182xaadc_mutex))
		return -EINTR;
	mutex_unlock(&cv182xaadc_mutex);
	pr_debug("%s\n", __func__);
	return 0;
}
...
static void cv182xaadc_clk_on(struct cv182xaadc *adc)
{

	u32 clk_ctrl0;

	mutex_lock(&cv182xaadc_mutex);
...
static void cv182xaadc_set_mclk(struct cv182xaadc *adc, u32 rate)
{
...
static void cv182xaadc_clk_off(struct cv182xaadc *adc)
{
	u32 i2s_en;
	u32 clk_ctrl0;
...
static int cv182xaadc_set_dai_fmt(struct snd_soc_dai *dai,
					unsigned int fmt)
{

	struct cv182xaadc *adc = snd_soc_dai_get_drvdata(dai);
...
static int cv182xaadc_hw_params(struct snd_pcm_substream *substream,
...
static int cv182xaadc_startup(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct cv182xaadc *adc = snd_soc_dai_get_drvdata(dai);

	dev_dbg(adc->dev, "%s\n", __func__);
	cv182xaadc_clk_on(adc);

	return 0;
}

static void cv182xaadc_on(struct cv182xaadc *adc)
{
...
static void cv182xaadc_off(struct cv182xaadc *adc)
{
...
static void cv182xaadc_shutdown(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct cv182xaadc *adc = snd_soc_dai_get_drvdata(dai);
	...
	cv182xaadc_off(adc);
	cv182xa_reset_adc();
	cv182xaadc_clk_off(adc);
}

static int cv182xaadc_trigger(struct snd_pcm_substream *substream,
...
static int cv182xaadc_prepare(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct cv182xaadc *adc = snd_soc_dai_get_drvdata(dai);
	u32 val;
...
static struct cv182xaadc *file_adc_dev(struct file *file)
{
	return container_of(file->private_data, struct cv182xaadc, miscdev);
}
...
static long adc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct cv182xaadc *adc = file_adc_dev(file);
	...
	if (argp != NULL) {
		if (!copy_from_user(&val, argp, sizeof(val))) {
			if (mutex_lock_interruptible(&cv182xaadc_mutex)) {
				pr_debug("cv182xaadc: signal arrives while waiting for lock\n");
				return -EINTR;
			}
		} else
			return -EFAULT;
	...
	case ACODEC_SET_ADCL_VOL:
		...
		if (copy_from_user(&vol, argp, sizeof(vol))) {
			if (mutex_is_locked(&cv182xaadc_mutex))
				mutex_unlock(&cv182xaadc_mutex);
...
static const struct snd_soc_dai_ops cv182xaadc_dai_ops = {
	.hw_params	= cv182xaadc_hw_params,
	.set_fmt	= cv182xaadc_set_dai_fmt,
	.startup	= cv182xaadc_startup,
	.shutdown	= cv182xaadc_shutdown,
	.trigger	= cv182xaadc_trigger,
	.prepare	= cv182xaadc_prepare,
};

static struct snd_soc_dai_driver cv182xaadc_dai = {
	.name		= "cv182xaadc",
	.capture	= {
		.stream_name	= "Capture",
		.channels_min	= 1,
	...
	.ops		= &cv182xaadc_dai_ops,
};

static const struct snd_kcontrol_new cv182xaadc_controls[] = {
...
unsigned int cv182xaadc_reg_read(struct snd_soc_component *codec, unsigned int reg)
{
	int ret, lidx, ridx;
	struct cv182xaadc *adc = dev_get_drvdata(codec->dev);
...
int cv182xaadc_reg_write(struct snd_soc_component *codec, unsigned int reg, unsigned int value)
{
	struct cv182xaadc *adc = dev_get_drvdata(codec->dev);
	u32 temp_lval;
	u32 temp_rval;
...
static const struct snd_soc_component_driver soc_component_dev_cv182xaadc = {
	.controls = cv182xaadc_controls,
	.num_controls = ARRAY_SIZE(cv182xaadc_controls),
	.read = cv182xaadc_reg_read,
	.write = cv182xaadc_reg_write,
};
...
static int adc_device_register(struct cv182xaadc *adc)
{
	struct miscdevice *miscdev = &adc->miscdev;
	int ret;

	miscdev->minor = MISC_DYNAMIC_MINOR;
	miscdev->name = "cv182xaadc";
...
static int cv182xaadc_probe(struct platform_device *pdev)
{
	struct cv182xaadc *adc;
	struct resource *res;
	...
	return devm_snd_soc_register_component(&pdev->dev, &soc_component_dev_cv182xaadc,
					  &cv182xaadc_dai, 1);
}

static int cv182xaadc_remove(struct platform_device *pdev)
{
	struct cv182xaadc *adc = dev_get_drvdata(&pdev->dev);
...
static int cv182xaadc_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct cv182xaadc *adc = platform_get_drvdata(pdev);

	if (!adc->reg_ctx) {
		adc->reg_ctx = devm_kzalloc(adc->dev, sizeof(struct cv182xaadc_context), GFP_KERNEL);
		if (!adc->reg_ctx)
			return -ENOMEM;
	}
...
static int cv182xaadc_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct cv182xaadc *adc = platform_get_drvdata(pdev);
...
static SIMPLE_DEV_PM_OPS(cv182xaadc_pm_ops, cv182xaadc_suspend,
			 cv182xaadc_resume);
#endif


static struct platform_driver cv182xaadc_platform_driver = {
	.probe		= cv182xaadc_probe,
	.remove		= cv182xaadc_remove,
	.driver		= {
		.name	= "cv182xaadc",
		.of_match_table = of_match_ptr(cvitek_adc_of_match),
#ifdef CONFIG_PM_SLEEP
		.pm	= &cv182xaadc_pm_ops,
#endif
	},
};
module_platform_driver(cv182xaadc_platform_driver);

MODULE_DESCRIPTION("ASoC CVITEK cv182xaADC driver");
MODULE_AUTHOR("Ethan Chen <ethan.chen@wisecore.com.tw>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:cv182xaadc");
