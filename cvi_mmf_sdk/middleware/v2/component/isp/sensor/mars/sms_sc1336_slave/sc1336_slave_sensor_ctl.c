...
const CVI_U8 sc1336_slave_i2c_addr = 0x32;        /* I2C Address of SC1336_SLAVE */
...
int sc1336_slave_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunsc1336_Slave_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, sc1336_slave_i2c_addr);
	if (ret < 0) {
...
int sc1336_slave_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (sc1336_slave_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, sc1336_slave_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, sc1336_slave_data_byte);
	if (ret < 0) {
...
int sc1336_slave_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (sc1336_slave_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, sc1336_slave_addr_byte + sc1336_slave_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
