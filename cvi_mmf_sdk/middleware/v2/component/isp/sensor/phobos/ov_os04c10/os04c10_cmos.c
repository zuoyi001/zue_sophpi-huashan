...
static CVI_S32 cmos_inttime_update(VI_PIPE ViPipe, CVI_U32 *u32IntTime)
{
	...
	if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
		...
		CVI_FLOAT ratio =
			(CVI_FLOAT)((u32IntTime[1] + 0.25) * (u32IntTime[0] + 0.75)) /  \
			(CVI_FLOAT)((u32IntTime[1] + 0.75) * (u32IntTime[0] + 0.25));
...
static CVI_S32 cmos_again_calc_table(VI_PIPE ViPipe, CVI_U32 *pu32AgainLin, CVI_U32 *pu32AgainDb)
{
	...
	if (*pu32AgainLin >= (int)(gain_table[63] * rate)) {
		*pu32AgainLin = (int)(gain_table[63] * rate);
		*pu32AgainDb = (63 - 48) * 64 + 1024;
		return CVI_SUCCESS;
	}

	for (i = 1; i < 64; i++) {
		if (*pu32AgainLin < (int)(gain_table[i] * rate)) {
			*pu32AgainLin = (int)(gain_table[i - 1] * rate);
			i--;
			if (i < 16)
				*pu32AgainDb = i * 8 + 128;
			else if (i < 32)
