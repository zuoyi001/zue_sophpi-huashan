...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	if (pstSnsState->u8ImgMode == PR2100_MODE_1080P) {
		if (ViPipe == 0) {
			memcpy(pstRxAttr, &pr2100_rx_attr, sizeof(*pstRxAttr));
			CVI_TRACE_SNS(CVI_DBG_INFO, "get pr2100_rx_attr\n");
		} else {
			memcpy(pstRxAttr, &pr2100_rx1_attr, sizeof(*pstRxAttr));
			CVI_TRACE_SNS(CVI_DBG_INFO, "get pr2100_rx1_attr\n");
		}
	} else {
		memcpy(pstRxAttr, &pr2100_multi_rx_attr, sizeof(*pstRxAttr));
		pstRxAttr->devno = ViPipe;
		CVI_TRACE_SNS(CVI_DBG_INFO, "get pr2100_multi_rx_attr %d\n", ViPipe);
	}
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &pr2100_rx_attr;
	int i;
