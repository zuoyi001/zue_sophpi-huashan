...
#define BM_DBG_ERR(msg, ...)    if (jpu_level & BM_MASK_ERR)    { printf("[ERR] %s = %d, "msg, __FUNCTION__, __LINE__, ## __VA_ARGS__); }
#define BM_DBG_FLOW(msg, ...)   if (jpu_level & BM_MASK_FLOW)   { printf("[FLOW] %s = %d, "msg, __FUNCTION__, __LINE__, ## __VA_ARGS__); }
#define BM_DBG_MEM(msg, ...)    if (jpu_level & BM_MASK_MEM)    { printf("[MEM] %s = %d, "msg, __FUNCTION__, __LINE__, ## __VA_ARGS__); }
#define BM_DBG_TRACE(msg, ...)  if (jpu_level & BM_MASK_TRACE)  { printf("[TRACE] %s = %d, "msg, __FUNCTION__, __LINE__, ## __VA_ARGS__); }
#define BM_DBG_PERF(msg, ...)   if (jpu_level & BM_MASK_PERF)  { printf("[PERF] %s = %d, "msg, __FUNCTION__, __LINE__, ## __VA_ARGS__); }
