				 ...
				 int checkeos, int *pstreameos, int endian)
{
	...
	if (ret != JPG_RET_SUCCESS) {
		JLOG(ERR,
		     "VPU_DecGetBitstreamBuffer failed Error code is 0x%x\n",
		     ret);
		goto FILL_BS_ERROR;
	}
	...
	if (*pstreameos == 0) {
		...
		if (ret != JPG_RET_SUCCESS) {
			JLOG(ERR,
			     "VPU_DecUpdateBitstreamBuffer failed Error code is 0x%x\n",
			     ret);
			goto FILL_BS_ERROR;
		}
...
int LoadYuvPartialImageHelperFormat(
				osal_file_t *yuvFp, Uint8 *pYuv, PhysicalAddress addrY,
				...
				int partPosIdx, int frameIdx, int packed)
{
	...
	if (format != FORMAT_400 && packed == 0) {
		...
		if (!OSAL_FREAD(pYuv + LumaPartialSize, 1, ChromaPartialSize,
			...
			if (!OSAL_FEOF(yuvFp))
				JLOG(ERR,
				     "Yuv Data fread failed file handle is 0x%p\n",
				     yuvFp);
			return 0;
		}
		...
		if (!OSAL_FREAD(pYuv + LumaPartialSize + ChromaPartialSize, 1,
			     ...
			     int format, int endian, int packed)
{
	...
	if (yuvFp) {
		if (!OSAL_FWRITE(pYuv, sizeof(Uint8), frameSize, yuvFp)) {
			JLOG(ERR,
			     "Frame Data fwrite failed file handle is 0x%p\n",
			     yuvFp);
			return 0;
		}
		OSAL_FFLUSH(yuvFp);
				...
				int partPosIdx, int frameIdx, int packed)
{
	...
	case FORMAT_420:
		...
		ChromaPartialSize =
		    ((picWidth + 1) / 2) * ((picHeightPartial + 1) / 2);
		if (interLeave)
			ChromaPartialSize = (((picWidth + 1) / 2) * 2) *
					    ((picHeightPartial + 1) / 2);
		break;
	case FORMAT_224:
		LumaPicSize = picWidth * ((picHeight + 1) / 2 * 2);
		...
		if (interLeave)
			ChromaPartialSize =
			    picWidth * 2 * ((picHeightPartial + 1) / 2);
		break;
	case FORMAT_422:
		LumaPicSize = picWidth * picHeight;
		...
		if (interLeave)
			ChromaPartialSize =
			    (((picWidth + 1) / 2) * 2) * picHeightPartial;
		break;
	case FORMAT_444:
		LumaPicSize = picWidth * picHeight;
	...
	if (yuvFp) {
		...
		if (format != FORMAT_400) {
			...
			pos = LumaPicSize + ChromaPartialSize * partPosIdx;
			OSAL_FSEEK(yuvFp, pos, SEEK_CUR);
			if (!OSAL_FWRITE(pYuv + LumaPartialSize, sizeof(Uint8),
					 ChromaPartialSize, yuvFp)) {
				JLOG(ERR,
				     "Frame Data fwrite failed file handle is 0x%p\n",
				     yuvFp);
			...
			OSAL_FSEEK(yuvFp, pos, SEEK_CUR);
			if (!OSAL_FWRITE(
				pYuv + LumaPartialSize + ChromaPartialSize,
		       ...
		       char *srcYuvFileName)
{
	...
	if (strlen(pEncConfig->strCfgDir))
		sprintf(cfgFileName, "%s%s", pEncConfig->strCfgDir,
	...
	if (srcYuvFileName) {
		if (strlen(pEncConfig->strStmDir))
			sprintf(srcYuvFileName, "%s%s", pEncConfig->strStmDir,
				encCfg.SrcFileName);
		//	else
		//		strcpy(srcYuvFileName, encCfg.SrcFileName);
	}
...
int parseJpgCfgFile(ENC_CFG *pEncCfg, char *FileName)
{
	...
	if (GetValue(Fp, "FRAME_FORMAT", sLine) == 0)
		return 0;
	ret = sscanf(sLine, "%d", &pEncCfg->FrmFormat);

	// width
	if (GetValue(Fp, "PICTURE_WIDTH", sLine) == 0)
		return 0;
	ret = sscanf(sLine, "%d", &pEncCfg->PicX);

	// height
	if (GetValue(Fp, "PICTURE_HEIGHT", sLine) == 0)
		return 0;
	ret = sscanf(sLine, "%d", &pEncCfg->PicY);
	...
	ret = sscanf(sLine, "%d", &pEncCfg->NumFrame);

	if (GetValue(Fp, "VERSION_ID", sLine) == 0)
		return 0;

	ret = sscanf(sLine, "%d", &pEncCfg->VersionID);

	if (GetValue(Fp, "RESTART_INTERVAL", sLine) == 0)
		return 0;

	ret = sscanf(sLine, "%d", &pEncCfg->RstIntval);

	if (GetValue(Fp, "IMG_FORMAT", sLine) == 0)
		return 0;

	ret = sscanf(sLine, "%d", &pEncCfg->SrcFormat);
	...
	if (format == YUV444) {
		for (j = 0; j < height; j++) {
			...
			for (i = 0; i < width; i++) {
				...
				(*cur) =
				    jpu_clip((298 * c + 409 * e + 128) >> 8);
				cur++;
				(*cur) = jpu_clip(
				    (298 * c - 100 * d - 208 * e + 128) >> 8);
				cur++;
				(*cur) =
				    jpu_clip((298 * c + 516 * d + 128) >> 8);
				cur++;
				(*cur) = 0;
				cur++;
			}
		 ...
		 int stride, int interleave, int format, int endian, int packed)
{
	...
	if (picWidth == stride) {
		...
		for (y = 0; y < nY; ++y) {
			if (OSAL_MEMCMP(pOrg + stride * y, pRef + y * picWidth,
					picWidth)) {
		...
		if (interleave) {
			...
			for (y = 0; y < nCb; ++y) {
				if (OSAL_MEMCMP(pOrg + chromaStride * y,
						pRef + y * chromaWidth,
						chromaWidth)) {
					JLOG(INFO,
