...
static void *my_table = NULL;
EXPORT LinearEchoState *LP_AEC_init(int frame_size, short filter_length, short init_filter_length, short *x_old, float Fs)
{
...
EXPORT void LP_AEC(LinearEchoState *st, const short *in, const short *far_end, short *out, short *save_mic_in)
{
...
#if 0
printf("dump 5-2 before DC_filter in[%d] radius[%f] frm_size[%d]\n", __LINE__, st->notch_radius, st->frame_size);
...
for (i = 0; i < 160; i++) {
   ...
   DC_filter(in, st->notch_radius, st->input, st->frame_size, st->notch_mem, save_mic_in);

//printf("replace xxxx[%d] radius[0x%x] [0x%x] [0x%x]\n", __LINE__, st->notch_radius, st->memD[0], st->memX[0]);
//for (i = 0; i < 160 ; i++) {
//   st->input[i] = in[i];
//   st->x[i] = st->x[2*i] = in[i];
//}

#if 0
printf("dump 14 task enter critical larger [%d] radius[0x%x]\n", __LINE__, st->notch_radius);

for (i = 0; i < 160; i++) {
      save_mic_in[i] = st->input[i];
}
#endif
#if 0
printf("dump 5 again inside LP_AEC  output[%d] radius[0x%x]\n", __LINE__, st->notch_radius);
...
#if 0
printf("dump 12 xxx inside LP_AEC fft input st->x,[%d]\n", __LINE__);

for (i = 0; i < 160; i++) {
      save_mic_in[i] =  st->x[i];
}
#endif
   speech_fft(st->fft_table, st->x, &st->X[0]);
#if 0
printf("dump 7 inside LP_AEC fft output xxx st->X[0],[%d]\n", __LINE__);
...
#if 0
printf("dump 8  inside LP_AEC output accum_power_spectrum,[%d]\n", __LINE__);

for (i = 0; i < 160; i++) {
      save_mic_in[i] =  st->Xf[i];
}
#endif
   Sff = 0;
#ifdef TWO_FILTER
...
#if 0
printf("dump 9  inside LP_AEC speech_ifft after[%d]\n", __LINE__);

for (i = 0; i < 160; i++) {
      save_mic_in[i] =  st->e[i];
}
#endif
   /* the variance of e(n) */
...
#if 0
printf("dump 10  inside LP_AEC aec_adjust_prop st->prop[%d]\n", __LINE__);

for (i = 0; i < M; i++) {
      save_mic_in[i] =  st->prop[i];
}
#endif
   }
   if (st->saturated == 0) {
