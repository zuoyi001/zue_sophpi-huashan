...
#include <dt-bindings/clock/mars-clock.h>

#define MARS_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE)
// #define MARS_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE | CLK_IS_CRITICAL)
// #define MARS_CLK_FLAGS_ALL	(CLK_GET_RATE_NOCACHE | CLK_IGNORE_UNUSED)
#define MARS_CLK_FLAGS_MUX	(CLK_SET_RATE_PARENT | CLK_SET_RATE_NO_REPARENT)
...
#define MARS_PLL_LOCK_TIMEOUT_MS	200
...
#define to_mars_pll_clk(_hw) container_of(_hw, struct mars_pll_hw_clock, hw)
#define to_mars_clk(_hw) container_of(_hw, struct mars_hw_clock, hw)

#define div_mask(width) ((1 << (width)) - 1)

static DEFINE_SPINLOCK(mars_clk_lock);

struct mars_clock_data {
	void __iomem *base;
	spinlock_t *lock;
...
struct mars_hw_clock {
	...
	struct mars_gate gate;
	struct mars_div div[2]; /* 0: DIV_IN0, 1: DIV_IN1 */
	struct mars_mux mux[3]; /* 0: bypass, 1: CLK_SEL, 2: CLK_SRC(DIV_IN0_SRC_MUX) */
};

struct mars_pll_clock {
	unsigned int	id;
	const char	*name;
	u32		reg_csr;
	u32		reg_ssc;
...
struct mars_pll_hw_clock {
	struct mars_pll_clock pll;
	void __iomem *base;
	spinlock_t *lock;
	struct clk_hw hw;
};

static const struct clk_ops mars_g6_pll_ops;
static const struct clk_ops mars_g2_pll_ops;
static const struct clk_ops mars_g2d_pll_ops;
static const struct clk_ops mars_clk_ops;

static struct mars_clock_data *clk_data;

static unsigned long cvi_clk_flags;

#define MARS_CLK(_id, _name, _parents, _gate_reg, _gate_shift,		\
...
const char *const mars_pll_parent[] = {"osc"};
const char *const mars_frac_pll_parent[] = {"clk_mipimpll"};

/*
 * All PLL clocks are marked as CRITICAL, hence they are very crucial
 * for the functioning of the SoC
 */
static struct mars_pll_hw_clock mars_pll_clks[] = {
	CLK_G6_PLL(MARS_CLK_MPLL, "clk_mpll", mars_pll_parent, REG_MPLL_CSR, 0),
	CLK_G6_PLL(MARS_CLK_TPLL, "clk_tpll", mars_pll_parent, REG_TPLL_CSR, 0),
	CLK_G6_PLL(MARS_CLK_FPLL, "clk_fpll", mars_pll_parent, REG_FPLL_CSR, 0),
	CLK_G2_PLL(MARS_CLK_MIPIMPLL, "clk_mipimpll", mars_pll_parent, REG_MIPIMPLL_CSR, 0, 0),
	CLK_G2_PLL(MARS_CLK_A0PLL, "clk_a0pll", mars_frac_pll_parent, REG_APLL0_CSR, REG_APLL_SSC_SYN_CTRL, 0),
	CLK_G2_PLL(MARS_CLK_DISPPLL, "clk_disppll", mars_frac_pll_parent, REG_DISPPLL_CSR, REG_DISPPLL_SSC_SYN_CTRL, 0),
	CLK_G2_PLL(MARS_CLK_CAM0PLL, "clk_cam0pll", mars_frac_pll_parent, REG_CAM0PLL_CSR, REG_CAM0PLL_SSC_SYN_CTRL,
		CLK_IGNORE_UNUSED),
	CLK_G2_PLL(MARS_CLK_CAM1PLL, "clk_cam1pll", mars_frac_pll_parent, REG_CAM1PLL_CSR, REG_CAM1PLL_SSC_SYN_CTRL, 0),
	CLK_G2D_PLL(MARS_CLK_MIPIMPLL_D3, "clk_mipimpll_d3", mars_pll_parent, REG_MIPIMPLL_CSR,
		0, 3, CLK_IGNORE_UNUSED),
	CLK_G2D_PLL(MARS_CLK_CAM0PLL_D2, "clk_cam0pll_d2", mars_frac_pll_parent, REG_CAM0PLL_CSR,
		REG_CAM0PLL_SSC_SYN_CTRL, 2, CLK_IGNORE_UNUSED),
	CLK_G2D_PLL(MARS_CLK_CAM0PLL_D3, "clk_cam0pll_d3", mars_frac_pll_parent, REG_CAM0PLL_CSR,
 ...
 * of the SoC.
 */
static struct mars_hw_clock mars_clks[] = {
	MARS_CLK(MARS_CLK_A53, "clk_a53",
		...
		REG_DIV_CLK_A53_0, 8,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CPU_AXI0, "clk_cpu_axi0",
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CPU_GIC, "clk_cpu_gic",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_XTAL_A53, "clk_xtal_a53",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 3,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_TPU, "clk_tpu",
		...
		REG_DIV_CLK_TPU, 8,
		0),
	MARS_CLK(MARS_CLK_TPU_FAB, "clk_tpu_fab",
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_AHB_ROM, "clk_ahb_rom",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 6,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_DDR_AXI_REG, "clk_ddr_axi_reg",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_0, 7,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_RTC_25M, "clk_rtc_25m",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 8,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TEMPSEN, "clk_tempsen",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 9,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SARADC, "clk_saradc",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 10,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_EFUSE, "clk_efuse",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_APB_EFUSE, "clk_apb_efuse",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_DEBUG, "clk_debug",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 13,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_XTAL_MISC, "clk_xtal_misc",
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 14,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_EMMC, "clk_axi4_emmc",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 15,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_EMMC, "clk_emmc",
		((const char *[]) {"osc", "clk_fpll", "clk_disppll"}),
		REG_CLK_EN_0, 16,
		...
		REG_DIV_CLK_EMMC, 8,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_100K_EMMC, "clk_100k_emmc",
		((const char *[]) {"clk_1m"}),
		REG_CLK_EN_0, 17,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_SD0, "clk_axi4_sd0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 18,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_SD0, "clk_sd0",
		...
		REG_DIV_CLK_SD0, 8,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_100K_SD0, "clk_100k_sd0",
		((const char *[]) {"clk_1m"}),
		REG_CLK_EN_0, 20,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_SD1, "clk_axi4_sd1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_SD1, "clk_sd1",
		...
		REG_DIV_CLK_SD1, 8,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_100K_SD1, "clk_100k_sd1",
		((const char *[]) {"clk_1m"}),
		REG_CLK_EN_0, 23,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SPI_NAND, "clk_spi_nand",
		((const char *[]) {"osc", "clk_fpll", "clk_disppll"}),
		REG_CLK_EN_0, 24,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_500M_ETH0, "clk_500m_eth0",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_0, 25,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_ETH0, "clk_axi4_eth0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 26,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_500M_ETH1, "clk_500m_eth1",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_0, 27,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_ETH1, "clk_axi4_eth1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_0, 28,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_GPIO, "clk_apb_gpio",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_0, 29,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_GPIO_INTR, "clk_apb_gpio_intr",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_0, 30,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_GPIO_DB, "clk_gpio_db",
		((const char *[]) {"clk_1m"}),
		REG_CLK_EN_0, 31,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AHB_SF, "clk_ahb_sf",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 0,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SDMA_AXI, "clk_sdma_axi",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 1,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SDMA_AUD0, "clk_sdma_aud0",
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SDMA_AUD1, "clk_sdma_aud1",
		((const char *[]) {"osc", "clk_a0pll", "a24k_clk"}),
		REG_CLK_EN_1, 3,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SDMA_AUD2, "clk_sdma_aud2",
		((const char *[]) {"osc", "clk_a0pll", "a24k_clk"}),
		REG_CLK_EN_1, 4,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SDMA_AUD3, "clk_sdma_aud3",
		((const char *[]) {"osc", "clk_a0pll", "a24k_clk"}),
		REG_CLK_EN_1, 5,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C, "clk_apb_i2c",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 6,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_WDT, "clk_apb_wdt",
		((const char *[]) {"osc"}),
		REG_CLK_EN_1, 7,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_PWM, "clk_pwm",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_SPI0, "clk_apb_spi0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 9,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_SPI1, "clk_apb_spi1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 10,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_SPI2, "clk_apb_spi2",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_SPI3, "clk_apb_spi3",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CAM0_200, "clk_cam0_200",
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_UART0, "clk_uart0",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_UART0, "clk_apb_uart0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 15,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_UART1, "clk_uart1",
		((const char *[]) {"clk_cam0_200"}),
		REG_CLK_EN_1, 16,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_UART1, "clk_apb_uart1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 17,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_UART2, "clk_uart2",
		((const char *[]) {"clk_cam0_200"}),
		REG_CLK_EN_1, 18,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_UART2, "clk_apb_uart2",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 19,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_UART3, "clk_uart3",
		((const char *[]) {"clk_cam0_200"}),
		REG_CLK_EN_1, 20,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_UART3, "clk_apb_uart3",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_UART4, "clk_uart4",
		((const char *[]) {"clk_cam0_200"}),
		REG_CLK_EN_1, 22,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_UART4, "clk_apb_uart4",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 23,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2S0, "clk_apb_i2s0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 24,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2S1, "clk_apb_i2s1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 25,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2S2, "clk_apb_i2s2",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 26,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2S3, "clk_apb_i2s3",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 27,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI4_USB, "clk_axi4_usb",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 28,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_APB_USB, "clk_apb_usb",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_1, 29,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_125M_USB, "clk_125m_usb",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_1, 30,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_33K_USB, "clk_33k_usb",
		((const char *[]) {"clk_1m"}),
		REG_CLK_EN_1, 31,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_12M_USB, "clk_12m_usb",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_2, 0,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_AXI4, "clk_axi4",
		((const char *[]) {"osc", "clk_fpll", "clk_disppll"}),
		REG_CLK_EN_2, 1,
		...
		REG_DIV_CLK_AXI4, 8,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI6, "clk_axi6",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_2, 2,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_DSI_ESC, "clk_dsi_esc",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_AXI_VIP, "clk_axi_vip",
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_SRC_VIP_SYS_0, "clk_src_vip_sys_0",
		((const char *[]) {"osc", "clk_mipimpll", "clk_cam0pll", "clk_disppll", "clk_fpll"}),
		REG_CLK_EN_2, 5,
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_SRC_VIP_SYS_1, "clk_src_vip_sys_1",
		((const char *[]) {"osc", "clk_mipimpll", "clk_cam0pll", "clk_disppll", "clk_fpll"}),
		REG_CLK_EN_2, 6,
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_DISP_SRC_VIP, "clk_disp_src_vip",
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_AXI_VIDEO_CODEC, "clk_axi_video_codec",
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_VC_SRC0, "clk_vc_src0",
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_H264C, "clk_h264c",
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_H265C, "clk_h265c",
		((const char *[]) {"clk_axi_video_codec"}),
		REG_CLK_EN_2, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_JPEG, "clk_jpeg",
		((const char *[]) {"clk_axi_video_codec"}),
		REG_CLK_EN_2, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_APB_JPEG, "clk_apb_jpeg",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_2, 13,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_APB_H264C, "clk_apb_h264c",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_2, 14,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_APB_H265C, "clk_apb_h265c",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_2, 15,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_CAM0, "clk_cam0",
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_CAM1, "clk_cam1",
		((const char *[]) {"clk_cam0pll", "clk_cam0pll_d2", "clk_cam0pll_d3", "clk_mipimpll_d3"}),
		REG_CLK_EN_2, 17,
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_CSI_MAC0_VIP, "clk_csi_mac0_vip",
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_CSI_MAC1_VIP, "clk_csi_mac1_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 19,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_ISP_TOP_VIP, "clk_isp_top_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 20,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_IMG_D_VIP, "clk_img_d_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_IMG_V_VIP, "clk_img_v_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 22,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SC_TOP_VIP, "clk_sc_top_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 23,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_SC_D_VIP, "clk_sc_d_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 24,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SC_V1_VIP, "clk_sc_v1_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 25,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SC_V2_VIP, "clk_sc_v2_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 26,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SC_V3_VIP, "clk_sc_v3_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 27,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_DWA_VIP, "clk_dwa_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 28,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_BT_VIP, "clk_bt_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 29,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_DISP_VIP, "clk_disp_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 30,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_DSI_MAC_VIP, "clk_dsi_mac_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_2, 31,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_LVDS0_VIP, "clk_lvds0_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 0,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_LVDS1_VIP, "clk_lvds1_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 1,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CSI0_RX_VIP, "clk_csi0_rx_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 2,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CSI1_RX_VIP, "clk_csi1_rx_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 3,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_PAD_VI_VIP, "clk_pad_vi_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 4,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_PM, "clk_pm",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER0, "clk_timer0",
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER1, "clk_timer1",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 10,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER2, "clk_timer2",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER3, "clk_timer3",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER4, "clk_timer4",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 13,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER5, "clk_timer5",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 14,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER6, "clk_timer6",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 15,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_TIMER7, "clk_timer7",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_3, 16,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C0, "clk_apb_i2c0",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 17,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C1, "clk_apb_i2c1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 18,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C2, "clk_apb_i2c2",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 19,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C3, "clk_apb_i2c3",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 20,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_I2C4, "clk_apb_i2c4",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	MARS_CLK(MARS_CLK_WGN, "clk_wgn",
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_WGN0, "clk_wgn0",
		((const char *[]) {"osc"}),
		REG_CLK_EN_3, 23,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_WGN1, "clk_wgn1",
		((const char *[]) {"osc"}),
		REG_CLK_EN_3, 24,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_WGN2, "clk_wgn2",
		((const char *[]) {"osc"}),
		REG_CLK_EN_3, 25,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_KEYSCAN, "clk_keyscan",
		((const char *[]) {"osc"}),
		REG_CLK_EN_3, 26,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_AHB_SF1, "clk_ahb_sf1",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_3, 27,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_VC_SRC1, "clk_vc_src1",
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_SRC_VIP_SYS_2, "clk_src_vip_sys_2",
		((const char *[]) {"osc", "clk_mipimpll", "clk_cam0pll", "clk_disppll", "clk_fpll"}),
		REG_CLK_EN_3, 29,
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_PAD_VI1_VIP, "clk_pad_vi1_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_3, 30,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CFG_REG_VIP, "clk_cfg_reg_vip",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_3, 31,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CFG_REG_VC, "clk_cfg_reg_vc",
		((const char *[]) {"clk_axi6"}),
		REG_CLK_EN_4, 0,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_AUDSRC, "clk_audsrc",
		((const char *[]) {"osc", "clk_a0pll", "a24k_clk"}),
		REG_CLK_EN_4, 1,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_APB_AUDSRC, "clk_apb_audsrc",
		((const char *[]) {"clk_axi4"}),
		REG_CLK_EN_4, 2,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_VC_SRC2, "clk_vc_src2",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_4, 3,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		0),
	MARS_CLK(MARS_CLK_PWM_SRC, "clk_pwm_src",
		((const char *[]) {"osc", "clk_fpll", "clk_disppll"}),
		REG_CLK_EN_4, 4,
		...
		0),
	MARS_CLK(MARS_CLK_AP_DEBUG, "clk_ap_debug",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_4, 5,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SRC_RTC_SYS_0, "clk_src_rtc_sys_0",
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_4, 6,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_PAD_VI2_VIP, "clk_pad_vi2_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 7,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CSI_BE_VIP, "clk_csi_be_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 8,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_VIP_IP0, "clk_vip_ip0",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 9,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_VIP_IP1, "clk_vip_ip1",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 10,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_VIP_IP2, "clk_vip_ip2",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_VIP_IP3, "clk_vip_ip3",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_C906_0, "clk_c906_0",
		((const char *[]) {"osc", "clk_fpll", "clk_tpll", "clk_a0pll", "clk_mipimpll", "clk_mpll"}),
		REG_CLK_EN_4, 13,
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_C906_1, "clk_c906_1",
		...
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_SRC_VIP_SYS_3, "clk_src_vip_sys_3",
		((const char *[]) {"osc", "clk_mipimpll", "clk_cam0pll", "clk_disppll", "clk_fpll"}),
		REG_CLK_EN_4, 15,
		...
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_SRC_VIP_SYS_4, "clk_src_vip_sys_4",
		((const char *[]) {"osc", "clk_mipimpll", "clk_cam0pll", "clk_disppll", "clk_fpll"}),
		REG_CLK_EN_4, 16,
		...
		0),
	MARS_CLK(MARS_CLK_IVE_VIP, "clk_ive_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 17,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_RAW_VIP, "clk_raw_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 18,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_OSDC_VIP, "clk_osdc_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 19,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IS_CRITICAL),
	MARS_CLK(MARS_CLK_CSI_MAC2_VIP, "clk_csi_mac2_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 20,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	MARS_CLK(MARS_CLK_CAM0_VIP, "clk_cam0_vip",
		((const char *[]) {"clk_axi_vip"}),
		REG_CLK_EN_4, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
...
static unsigned long mars_pll_rate_calc(u32 regval, s16 post_div_sel, unsigned long parent_rate)
{
	u64 numerator;
...
static unsigned long mars_g6_pll_recalc_rate(struct clk_hw *hw,
					       unsigned long parent_rate)
{
	struct mars_pll_hw_clock *pll_hw = to_mars_pll_clk(hw);
	unsigned long rate;
	u32 regval;

	regval = readl(pll_hw->base + pll_hw->pll.reg_csr);
	rate = mars_pll_rate_calc(regval, pll_hw->pll.post_div_sel, parent_rate);

	return rate;
}

static long mars_g6_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				     unsigned long *prate)
{
	return rate;
}

static int mars_g6_pll_calc_csr(unsigned long parent_rate, unsigned long rate, u32 *csr)
{
	u64 numerator;
	u32 denominator;
...
static int mars_g6_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long parent_rate)
{
	struct mars_pll_hw_clock *pll_hw = to_mars_pll_clk(hw);
	...
	ret = mars_g6_pll_calc_csr(parent_rate, rate, &regval_csr);
	if (ret < 0)
		return ret;
	...
	timeout = ktime_add_ms(ktime_get(), MARS_PLL_LOCK_TIMEOUT_MS);
	while (1) {
...
static const struct clk_ops mars_g6_pll_ops = {
	.recalc_rate = mars_g6_pll_recalc_rate,
	.round_rate = mars_g6_pll_round_rate,
	.set_rate = mars_g6_pll_set_rate,
};

static unsigned long mars_g2_pll_recalc_rate(struct clk_hw *hw,
					       unsigned long parent_rate)
{
	struct mars_pll_hw_clock *pll_hw = to_mars_pll_clk(hw);
	u32 reg_ssc_set;
...
rate_calc:
	rate = mars_pll_rate_calc(regval_csr, pll_hw->pll.post_div_sel, clk_ref);
...
static int mars_g2_pll_get_setting_from_lut(unsigned long rate, u32 *csr,
					      u32 *ssc_set)
{
	int i;
...
static long mars_g2_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				     unsigned long *prate)
{
	return rate;
}

static int mars_g2_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long parent_rate)
{
	struct mars_pll_hw_clock *pll_hw = to_mars_pll_clk(hw);
	unsigned long flags = 0;
	...
	ret = mars_g2_pll_get_setting_from_lut(rate, &regval_csr,
						 &regval_ssc_set);
	if (ret < 0)
		return ret;
...
static const struct clk_ops mars_g2_pll_ops = {
	.recalc_rate = mars_g2_pll_recalc_rate,
	.round_rate = mars_g2_pll_round_rate,
	.set_rate = mars_g2_pll_set_rate,
};
...
static struct clk_hw *mars_clk_register_pll(struct mars_pll_hw_clock *pll_clk,
...
static void mars_clk_unregister_pll(struct clk_hw *hw)
{
	struct mars_pll_hw_clock *pll_hw = to_mars_pll_clk(hw);
...
static int mars_clk_register_plls(struct mars_pll_hw_clock *clks,
				    int num_clks,
				    struct mars_clock_data *data)
{
	struct clk_hw *hw;
	...
	for (i = 0; i < num_clks; i++) {
		struct mars_pll_hw_clock *mars_clk = &clks[i];

		hw = mars_clk_register_pll(mars_clk, pll_base);
		if (IS_ERR(hw)) {
			pr_err("%s: failed to register clock %s\n",
			       __func__, mars_clk->pll.name);
			goto err_clk;
		}

		data->hw_data.hws[clks[i].pll.id] = hw;

		clk_hw_register_clkdev(hw, mars_clk->pll.name, NULL);
	}
...
err_clk:
	while (i--)
		mars_clk_unregister_pll(data->hw_data.hws[clks[i].pll.id]);

	return PTR_ERR(hw);
}

static int mars_clk_is_bypassed(struct mars_hw_clock *clk_hw)
{
	u32 val;
...
static int mars_clk_get_clk_sel(struct mars_hw_clock *clk_hw)
{
	u32 val;
...
static int mars_clk_get_src_sel(struct mars_hw_clock *clk_hw)
{
	u32 val;
...
static unsigned long mars_clk_div_recalc_rate(struct clk_hw *hw,
					      unsigned long parent_rate)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	unsigned int clk_sel = mars_clk_get_clk_sel(clk_hw);
	void __iomem *reg_addr = clk_hw->base + clk_hw->div[clk_sel].reg;
	unsigned int val;
	unsigned long rate;

	if ((clk_hw->mux[0].shift >= 0) && mars_clk_is_bypassed(clk_hw))
		return parent_rate;
...
static long mars_clk_div_round_rate(struct clk_hw *hw, unsigned long rate,
				      unsigned long *prate)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	unsigned int clk_sel = mars_clk_get_clk_sel(clk_hw);
...
static long mars_clk_div_calc_round_rate(struct clk_hw *hw, unsigned long rate,
					 unsigned long *prate)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
...
static int mars_clk_div_determine_rate(struct clk_hw *hw,
				       struct clk_rate_request *req)
{
	...
	if ((parent_count < 2) || (clk_hw_get_flags(hw) & CLK_SET_RATE_NO_REPARENT)) {
		rate = mars_clk_div_round_rate(hw, req->rate, &req->best_parent_rate);
		if (rate < 0)
			return rate;
	...
	best_rate = mars_clk_div_calc_round_rate(hw, req->rate, &parent_rate);
	...
	for (which = 0; which < parent_count; which++) {
		...
		other_rate = mars_clk_div_calc_round_rate(hw, req->rate, &parent_rate);
...
static int mars_clk_div_set_rate(struct clk_hw *hw, unsigned long rate,
				 unsigned long parent_rate)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	unsigned int clk_sel = mars_clk_get_clk_sel(clk_hw);
...
static void mars_clk_gate_endisable(struct clk_hw *hw, int enable)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
...
static int mars_clk_gate_enable(struct clk_hw *hw)
{
	mars_clk_gate_endisable(hw, 1);

	return 0;
}

static void mars_clk_gate_disable(struct clk_hw *hw)
{
	mars_clk_gate_endisable(hw, 0);
}

static int mars_clk_gate_is_enabled(struct clk_hw *hw)
{
	u32 reg;
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	void __iomem *reg_addr = clk_hw->base + clk_hw->gate.reg;
...
static u8 mars_clk_mux_get_parent(struct clk_hw *hw)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	u8 clk_sel = mars_clk_get_clk_sel(clk_hw);
	u8 src_sel = mars_clk_get_src_sel(clk_hw);
	u8 parent_idx = 0;
	...
	if (clk_hw->mux[0].shift >= 0) {
		// clk with bypass reg
		if (mars_clk_is_bypassed(clk_hw)) {
			parent_idx = 0;
		} else {
			if (clk_hw->mux[1].shift >= 0) {
...
static int mars_clk_mux_set_parent(struct clk_hw *hw, u8 index)
{
	struct mars_hw_clock *clk_hw = to_mars_clk(hw);
	unsigned long flags = 0;
...
static const struct clk_ops mars_clk_ops = {
	// gate
	.enable = mars_clk_gate_enable,
	.disable = mars_clk_gate_disable,
	.is_enabled = mars_clk_gate_is_enabled,

	// div
	.recalc_rate = mars_clk_div_recalc_rate,
	.round_rate = mars_clk_div_round_rate,
	.determine_rate = mars_clk_div_determine_rate,
	.set_rate = mars_clk_div_set_rate,

	//mux
	.get_parent = mars_clk_mux_get_parent,
	.set_parent = mars_clk_mux_set_parent,
};

static struct clk_hw *mars_register_clk(struct mars_hw_clock *mars_clk,
					  void __iomem *sys_base)
{
	struct clk_hw *hw;
	struct clk_init_data init;
	...
	mars_clk->gate.flags = 0;
	mars_clk->div[0].flags = CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO |
				 CLK_DIVIDER_ROUND_CLOSEST;
	mars_clk->div[1].flags = CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO |
				 CLK_DIVIDER_ROUND_CLOSEST;
	mars_clk->mux[0].flags = 0; /* clk byp */
	mars_clk->mux[1].flags = 0; /* clk sel */
	mars_clk->mux[2].flags = 0; /* div_0 src_sel */
	mars_clk->base = sys_base;
	mars_clk->lock = &mars_clk_lock;

	if (cvi_clk_flags) {
		/* copy clk_init_data for modification */
		memcpy(&init, mars_clk->hw.init, sizeof(init));

		init.flags |= cvi_clk_flags;
		mars_clk->hw.init = &init;
	}
...
static void mars_unregister_clk(struct clk_hw *hw)
{
	struct mars_hw_clock *mars_clk = to_mars_clk(hw);

	clk_hw_unregister(hw);
	kfree(mars_clk);
}
static int mars_register_clks(struct mars_hw_clock *clks,
				int num_clks,
				struct mars_clock_data *data)
{
	struct clk_hw *hw;
	...
	for (i = 0; i < num_clks; i++) {
		struct mars_hw_clock *mars_clk = &clks[i];

		hw = mars_register_clk(mars_clk, sys_base);

		if (IS_ERR(hw)) {
			pr_err("%s: failed to register clock %s\n",
			       __func__, mars_clk->name);
			goto err_clk;
		}
		data->hw_data.hws[clks[i].id] = hw;
		clk_hw_register_clkdev(hw, mars_clk->name, NULL);
	}
...
err_clk:
	while (i--)
		mars_unregister_clk(data->hw_data.hws[clks[i].id]);
...
#ifdef CONFIG_PM_SLEEP
static int mars_clk_suspend(void)
{
...
static void mars_clk_resume(void)
{
	uint32_t regval;
...
static struct syscore_ops mars_clk_syscore_ops = {
	.suspend = mars_clk_suspend,
	.resume = mars_clk_resume,
};
#endif /* CONFIG_PM_SLEEP */
...
static void __init cvi_clk_init(struct device_node *node)
{
	...
	num_clks = ARRAY_SIZE(mars_pll_clks) +
		   ARRAY_SIZE(mars_clks);

	clk_data = kzalloc(sizeof(struct mars_clock_data) +
	...
	clk_data->lock = &mars_clk_lock;

	clk_data->base = of_iomap(node, 0);
	if (!clk_data->base) {
		pr_err("Failed to map address range for cvitek,mars-clk node\n");
		return;
	}

	mars_clk_register_plls(mars_pll_clks,
			       ARRAY_SIZE(mars_pll_clks),
			       clk_data);

	mars_register_clks(mars_clks,
			   ARRAY_SIZE(mars_clks),
			   clk_data);
	...
	/* force enable clocks */
	clk_prepare_enable(clk_data->hw_data.hws[MARS_CLK_DSI_MAC_VIP]->clk);
	clk_prepare_enable(clk_data->hw_data.hws[MARS_CLK_DISP_VIP]->clk);
	clk_prepare_enable(clk_data->hw_data.hws[MARS_CLK_BT_VIP]->clk);
	clk_prepare_enable(clk_data->hw_data.hws[MARS_CLK_SC_TOP_VIP]->clk);

#ifdef CONFIG_PM_SLEEP
	register_syscore_ops(&mars_clk_syscore_ops);
#endif
...
CLK_OF_DECLARE(cvi_clk, "cvitek,mars-clk", cvi_clk_init);
