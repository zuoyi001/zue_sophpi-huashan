...
struct mars_thermal_zone {
	unsigned int ch;
	void __iomem *base;
	struct mars_thermal *ct;
};
...
static void mars_thermal_init(struct mars_thermal *ct)
{
...
static void mars_thermal_uninit(struct mars_thermal *ct)
{
	void __iomem *base = ct->base;
	u32 regval;
...
static int mars_read_temp(void *data, int *temperature)
{
	struct mars_thermal_zone *ctz = data;
...
static const struct thermal_zone_of_device_ops mars_thermal_ops = {
	.get_temp = mars_read_temp,
};

static const struct of_device_id mars_thermal_of_match[] = {
	{
		.compatible = "cvitek,mars-thermal",
	},
	{},
};
MODULE_DEVICE_TABLE(of, mars_thermal_of_match);

static int mars_thermal_probe(struct platform_device *pdev)
{
	struct mars_thermal *ct;
	struct mars_thermal_zone *ctz;
	struct resource *res;
...
static int mars_thermal_remove(struct platform_device *pdev)
{
	struct mars_thermal *ct = platform_get_drvdata(pdev);

	mars_thermal_uninit(ct);
...
static int mars_thermal_suspend(struct device *dev)
{
	struct mars_thermal *ct = dev_get_drvdata(dev);
...
static int mars_thermal_resume(struct device *dev)
{
	struct mars_thermal *ct = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(mars_thermal_pm_ops,
			 mars_thermal_suspend, mars_thermal_resume);

static struct platform_driver mars_thermal_driver = {
	.probe = mars_thermal_probe,
	.remove = mars_thermal_remove,
	.driver = {
		.name = "mars-thermal",
		.pm = &mars_thermal_pm_ops,
		.of_match_table = mars_thermal_of_match,
	},
};

module_platform_driver(mars_thermal_driver);
