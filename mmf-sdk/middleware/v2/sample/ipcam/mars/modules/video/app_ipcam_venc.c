...
int fpStreamingSaveToFlash(void *pData, void *pArgs)
{
    ...
    if(access("/tmp/rec", F_OK) == 0) {
        if (pstVencChnCfg->pFile == NULL) {
            char szPostfix[8] = {0};
            char szFilePath[32] = {0};
            app_ipcam_Postfix_Get(pstVencChnCfg->enType, szPostfix);
            snprintf(szFilePath, 32, "%s/Venc%d_idx_%d%s", pstVencChnCfg->SavePath, pstVencChnCfg->VencChn, pstVencChnCfg->frameNum++, szPostfix);
...
int fpStreamingSendToRecord(void *pData, void *pArgs)
{
    ...
    if (VencChn == 0) {
        ...
        stFrameNum++;
        app_ipcam_Record_VideoInput(pstVencChnCfg->enType, pstStream, stFrameNum);
    }
    #endif
...
static void *Thread_Streaming_Proc(void *pArgs)
{
    ...
    while (pastVencChnCfg->bStart) {
        ...
        if (app_ipcam_JpgCapFlag_Get() && (vpssChn == 1)) {
        ...
        if (s32Ret != CVI_SUCCESS || (0 == stStream.u32PackCount)) {
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_VENC_GetStream, VencChn(%d) cnt(%d), s32Ret = 0x%X\n", VencChn, stStream.u32PackCount, s32Ret);
...
int app_ipcam_Venc_Init(APP_VENC_CHN_E VencIdx)
{
    ...
    for (VENC_CHN s32ChnIdx = 0; s32ChnIdx < g_pstVencCtx->s32VencChnCnt; s32ChnIdx++) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_WARN,"u32Profile [%d]\n", pstVencChnAttr->stVencAttr.u32Profile);
        ...
        #ifdef SLICE_BUFF_MODE
        if ((s32ChnIdx == 1) && (pstVencChnCfg->enBindMode != VENC_BIND_DISABLE)) {
...
int app_ipcam_VencResize_Stop(void)
{
    ...
    app_ipcam_LList_Data_DeInit(g_pDataCtx);
...
int app_ipcam_VencResize_Start(void)
{
    ...
    APP_CHK_RET(app_ipcam_Vpss_Create(), "vpss create");

    APP_CHK_RET(app_ipcam_Vpss_Start(), "vpss start");
    ...
    APP_CHK_RET(app_ipcam_Vpss_Bind_Vpss(), "init vpss bind vpss");
...
{
    /*                 stop venc flow                */
    APP_CHK_RET(app_ipcam_VencResize_Stop(), "video resize stop");

    /*          update venc parameter          */
    APP_CHK_RET(app_ipcam_CmdTask_VideoAttr_Parse(msg, userdate), "video Attr Parse");

    /*                 re-start venc flow                */
    APP_CHK_RET(app_ipcam_VencResize_Start(), "video resize start");
