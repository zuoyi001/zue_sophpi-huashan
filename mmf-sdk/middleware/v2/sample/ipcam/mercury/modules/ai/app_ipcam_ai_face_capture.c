...
#define FACE_CAPTURE_FILENAME 		  "/mnt/sd/capture_face_%llu_%f.png"
#define MAIN_CAPTURE_FILENAME         "/mnt/sd/capture_main_%llu_%f.png"
...
{
    .thr_size_min = 64,         /*face size less than it, quality = 0*/
    .thr_size_max = 400,        /*face size more than it, quality = 0*/
    .qa_method = 0,                /*use fq?*/
    .thr_quality = 0.0,            /*capture new face quality difference*/
    ...
    .fast_m_interval = 10,        /*fast mode capture time(fps)*/
    .fast_m_capture_num = 3,    /*fast mode how many face capture */
...
CVI_S32 app_ipcam_Ai_Face_Capure_Init(cviai_handle_t *handle)
{
        ...
        CVI_AI_APP_FaceCapture_SetMode(g_stFaceCaptureHandle, AUTO);
        ...
        g_stFaceCaptureHandle->face_cpt_info->fd_inference = CVI_AI_FaceMaskDetection;
...
CVI_S32 app_ipcam_Ai_Face_Capure(VIDEO_FRAME_INFO_S *stfdFrame,cvai_face_t *capture_face)
{
    ...
    {
        if (g_stFaceCaptureHandle->face_cpt_info->data[i].image.height > 0)
        {
            if ((g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.score * 100) < FACE_SCORE_THRESHOLD)
					continue;
            APP_PROF_LOG_PRINT(LEVEL_DEBUG, "unique_id:%llu imgae[w:%u h:%u stride:(%u %u %u) quality:%f score:%f length:(%u %u %u)]\n",
            ...
            /*filter capture face*/
            // float h = g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.y2 - g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.y1;
            // float w = g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.x2 - g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.x1;
            // if (((g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.score * 100) < FACE_SCORE_THRESHOLD) ||
            //     ((h < g_stFaceCaptureCfg.thr_size_min) || (w < g_stFaceCaptureCfg.thr_size_min)) ||
            //     ((h > g_stFaceCaptureCfg.thr_size_max) || (w > g_stFaceCaptureCfg.thr_size_max)) ||
            //     (abs(g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.yaw) > g_stFaceCaptureCfg.thr_yaw) ||
            //     (abs(g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.pitch) > g_stFaceCaptureCfg.thr_pitch) ||
            //     (abs(g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.roll) > g_stFaceCaptureCfg.thr_roll))
            // {
            //     APP_PROF_LOG_PRINT(LEVEL_INFO, "unique_id:%llu quality:%f score:%f yaw/pitch/roll:(%f %f %f) imgae[w:%f h:%f]\n",
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.unique_id,
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.face_quality,
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.bbox.score,
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.yaw,
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.pitch,
            //     g_stFaceCaptureHandle->face_cpt_info->data[i].info.head_pose.roll,
            //     h, w);
            //     continue;
            // }
            // else
            // {
                ...
                stbi_write_png(captureFileName, g_stFaceCaptureHandle->face_cpt_info->data[i].image.width,
                g_stFaceCaptureHandle->face_cpt_info->data[i].image.height, STBI_rgb,
                g_stFaceCaptureHandle->face_cpt_info->data[i].image.pix[0], g_stFaceCaptureHandle->face_cpt_info->data[i].image.stride[0]);

#if 0
                /*main picture*/
                stfdFrame.stVFrame.pu8VirAddr[0] = CVI_SYS_Mmap(stfdFrame.stVFrame.u64PhyAddr[0], stfdFrame.stVFrame.u32Length[0]);
                memset(captureFileName, 0, sizeof(captureFileName));
                snprintf(captureFileName, sizeof(captureFileName), MAIN_CAPTURE_FILENAME ,
                g_stFaceCaptureHandle->face_cpt_info->data[i].info.unique_id , g_stFaceCaptureHandle->face_cpt_info->data[i].info.face_quality);
                stbi_write_png(captureFileName, stfdFrame.stVFrame.u32Width,
                stfdFrame.stVFrame.u32Height, STBI_rgb,
                stfdFrame.stVFrame.pu8VirAddr[0], stfdFrame.stVFrame.u32Stride[0]);
                CVI_SYS_Munmap(stfdFrame.stVFrame.pu8VirAddr[0], stfdFrame.stVFrame.u32Length[0]);
#endif
                g_stFaceCaptureHandle->face_cpt_info->data[i].image.height = 0;
            //}
        }
    }
    return s32Ret;
