...
static int app_ipcam_Audio_ParamChcek(APP_AUDIO_CFG_S *pstAudioCfg, APP_AUDIO_VQE_S *pstAudioVqe)
{
    ...
    if (((pstAudioCfg->u32ChnCnt == 1) && (pstAudioCfg->enSoundmode == AUDIO_SOUND_MODE_STEREO)) ||
        ((pstAudioCfg->u32ChnCnt == 2) && (pstAudioCfg->enSoundmode == AUDIO_SOUND_MODE_MONO)))
    {
    ...
    if (pstAudioVqe->bAiAecEnable && pstAudioCfg->u32ChnCnt != 2)
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "audio param aec chn(%d) must be 2\n", pstAudioCfg->u32ChnCnt);
        return -1;
    }
...
static CVI_VOID *Thread_AudioAo_Proc(CVI_VOID *pArgs)
{
    ...
    while (mAudioAoThread.bRun_flag)
    {
        ...
        {
            app_ipcam_Audio_AoPlay(g_stAudioPlay.cAencFileName, AUDIO_AO_PLAY_TYPE_AENC);
        }
        if (gst_SocketFd > 0)
        {
    ...
    while (mAudioAencThread.bRun_flag)
    {
        ...
        if (stStream.u32Len > 0)
        {
            ...
            if (g_stAudioRecord.iStatus)
            {
                if (fp_recex == NULL)
                {
                    fp_recex = fopen(g_stAudioRecord.cAencFileName, "wb");
...
static CVI_VOID *Thread_AudioAi_Proc(CVI_VOID *pArgs)
{
    ...
    CVI_S32 s32OutputChnCnt = pastAudioCfg->u32ChnCnt;
    CVI_U32 iAiRecordTime = 0;
    CVI_U32 iAudioByte = 0;
    ...
    while (mAudioAiThread.bRun_flag)
    {
        ...
        s32Ret = CVI_AI_GetFrame(pastAudioCfg->u32AiDevId, pastAudioCfg->u32AiChn, &stFrame, &stAecFrm, -1);
        if (s32Ret != CVI_SUCCESS)
        {
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_AI_GetFrame CVI_AI_GetFrame None!!\n");
            continue;
        }
            ...
            if (iAiRecordTime)
            {
                ...
                if (fp_rec)
                {
                    ...
                    else
                    {
                        ...
                        {
                            iAudioByte = stFrame.u32Len * s32OutputChnCnt * 2;
                        }
                        else
                        {
                            iAudioByte = stFrame.u32Len * s32OutputChnCnt * 4;
                        }
                        fwrite(stFrame.u64VirAddr[0], 1, iAudioByte, fp_rec);
                        if (pastAudioCfg->Cal_DB_Enable)
                        {
                            float Db_value = app_ipcam_Audio_Calculate_DB((short *)stFrame.u64VirAddr[0], stFrame.u32Len * s32OutputChnCnt);
                            if (Db_value > 150)
                            {
...
static CVI_S32 app_ipcam_Audio_CfgAcodec()
{
    ...
    /* ao gpio enable*/
    app_ipcam_Speaker_Enable(SPEAKER_GPIO, CVI_TRUE);
...
static CVI_S32 app_ipcam_Audio_AdecStart(APP_AUDIO_CFG_S *pstAudioCfg, APP_AUDIO_VQE_S *pstAudioVqe)
{
    ...
    stAdecAttr.s32ChannelNums       = pstAudioVqe->bAiAecEnable ? 1 : pstAudioCfg->u32ChnCnt;
...
static CVI_S32 app_ipcam_Audio_AiStop(APP_AUDIO_CFG_S *pstAudioCfg, APP_AUDIO_VQE_S *pstAudioVqe)
{
    ...
    if (g_stAudioStatus.bAiInit)
    {
        ...
        for(i = 0; i <= (CVI_S32)pstAudioCfg->u32AiChn; i++)
        {
            if (i == 0)
            {
        ...
        if (i == 0)
        {
            if (pstAudioVqe->bAiAgcEnable || pstAudioVqe->bAiAnrEnable || pstAudioVqe->bAiAecEnable)
            {
                    ...
                    (pstAudioCfg->enSamplerate == AUDIO_SAMPLE_RATE_16000))
                {
                    ...
                    {
                        pstAiVqeTalkAttr->u32OpenMask |= AI_TALKVQE_MASK_AEC;
                        pstAiVqeTalkAttr->stAecCfg    = pstAudioVqe->mAiAecCfg;
                    }
                    pstAiVqeTalkAttr->s32FrameSample    = pstAudioCfg->u32PtNumPerFrm;
                    pstAiVqeTalkAttr->s32WorkSampleRate = pstAudioCfg->enSamplerate;
                    pstAiVqeTalkAttr->s32BytesPerSample = (pstAudioCfg->enBitwidth + 1);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "s32FrameSample[%d]\n", pstAiVqeTalkAttr->s32FrameSample);
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "AGC PARAM---------------------------------------------\n");
                            ...
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "para_agc_vad_enable[%d]\n", pstAiVqeTalkAttr->stAgcCfg.para_agc_vad_enable);
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "para_agc_vad_cnt[%d]\n", pstAiVqeTalkAttr->stAgcCfg.para_agc_vad_cnt);
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "para_agc_cut6_enable[%d]\n", pstAiVqeTalkAttr->stAgcCfg.para_agc_cut6_enable);
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "ANR PARAM---------------------------------------------\n");
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "para_nr_snr_coeff[%d]\n", pstAiVqeTalkAttr->stAnrCfg.para_nr_snr_coeff);
                            APP_PROF_LOG_PRINT(LEVEL_INFO, "para_nr_noise_coeff[%d]\n", pstAiVqeTalkAttr->stAnrCfg.para_nr_noise_coeff);
    ...
    while (NULL != temp)
    {
        ...
        switch (*temp)
        {
            case 'i': 
            {
                ...
                pstAudioCfg->astAudioCfg.enSoundmode =
                    (pstAudioCfg->astAudioCfg.u32ChnCnt == 1) ?  AUDIO_SOUND_MODE_MONO : AUDIO_SOUND_MODE_STEREO;
...
static int app_ipcam_Audio_IntercomStart(char *pstTargetIp, int iTargetPort)
{
    ...
    if (NULL == pstTargetIp)
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "pstTargetIp is NULL!\n");
        return s32Ret;
    }
...
int app_ipcam_Audio_AoPlay(char *pAudioFile, AUDIO_AO_PLAY_TYPE_E eAoType)
{
    ...
    do
    {
        if (eAoType == AUDIO_AO_PLAY_TYPE_AENC || eAoType == AUDIO_AO_PLAY_TYPE_INTERCOM)
        {
            ...
            {
                s32Ret = CVI_ADEC_SendEndOfStream(pstAudioCfg->u32AdChn, CVI_FALSE);
                if (s32Ret != CVI_SUCCESS)
                {
                    APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_ADEC_SendEndOfStream failed with %#x!\n", s32Ret);
                }
        ...
        else if (eAoType == AUDIO_AO_PLAY_TYPE_MP3)
        {
            ...
            CVI_S32 s32OutLen = 0;
            CVI_CHAR *pOutPutBuffer = (CVI_CHAR *)malloc(stAudioStream.u32Len * 20);
