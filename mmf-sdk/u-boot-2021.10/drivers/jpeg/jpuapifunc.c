...
int JpgDecHuffTabSetUp(JpgDecInfo *jpg)
{
	...
	for (i = 0; i < HuffLength;
	     i++) { // 8-bit, 12 row, 1 category (DC Chroma)
	...
	for (i = 0; i < HuffLength;
	     i++) { // 8-bit, 162 row, 1 category (AC Luma)
	...
	for (i = 0; i < HuffLength;
	     i++) { // 8-bit, 162 row, 1 category (AC Chroma)
...
int JpegDecodeConcealError(JpgDecInfo *jpg)
{
	...
	for (;;) {
		...
		if (iRstIdx >= 0 && iRstIdx <= 7) {
			...
			jpg->gbuStartPtr =
			    ((ecsPtr - jpg->frameOffset) / 256) * 256;
...
int JpegDecodeHeader(JpgDecInfo *jpg)
{
	...
	if (!jpg->ecsPtr) {
		...
		if (jpg->consumeByte != 0) { // meaning is frameIdx > 0
			...
			if (nextOffset <= 0)
				nextOffset =
				    2; // in order to consume start code.
		}
	...
	if (jpg->headerSize > 0 &&
	    (jpg->headerSize >
	     (jpg->streamBufSize -
	      jpg->frameOffset))) { // if header size is smaller than room of
				    // stream end. copy the buffer to bistream
				    // start.
		BM_DBG_ERR("\n");
	...
	for (;;) {
		...
		case SOS_Marker:
			...
			if (!jpg->headerSize)
				jpg->headerSize =
				    jpg->ecsPtr; // we assume header size of all
						 // frame is same for mjpeg case
			goto DONE_DEC_HEADER;
//			break;
		case EOI_Marker:
			goto DONE_DEC_HEADER;
...
JpgRet CheckJpgEncParam(JpgEncHandle handle, JpgEncParam *param)
{
	...
	if (pEncInfo->packedFormat != PACKED_FORMAT_NONE) {
		if (pEncInfo->packedFormat == PACKED_FORMAT_444) {
			if (param->sourceFrame->stride <
			    pEncInfo->openParam.picWidth * 2) {
				return JPG_RET_INVALID_PARAM;
			}
		}
		if (pEncInfo->packedFormat == PACKED_FORMAT_444) {
