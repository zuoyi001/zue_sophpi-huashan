...
#if LV_USE_BTNM == 0
#error "ime_kb: lv_btnm is required. Enable it in lv_conf.h (LV_USE_BTNM  1) "
#endif

#if LV_USE_TA == 0
#error "ime_kb: lv_ta is required. Enable it in lv_conf.h (LV_USE_TA  1) "
#endif
...
enum {
    IME_KB_MODE_TEXT,
    IME_KB_MODE_NUM,
    IME_KB_MODE_TEXT_UPPER,
    IME_KB_MODE_SPEC,
    IME_KB_MODE_CH
};
typedef uint8_t ime_kb_mode_t;
...
typedef struct
{
    ...
    ime_kb_mode_t mode;      /*Key map type*/
    uint8_t cursor_mng : 1; /*1: automatically show/hide cursor when a text area is assigned or left*/
} ime_kb_ext_t;

enum {
    IME_KB_STYLE_BG,
    IME_KB_STYLE_BTN_REL,
    IME_KB_STYLE_BTN_PR,
    IME_KB_STYLE_BTN_TGL_REL,
    IME_KB_STYLE_BTN_TGL_PR,
    IME_KB_STYLE_BTN_INA,
};
typedef uint8_t ime_kb_style_t;
 ...
 */
static inline void ime_kb_set_map(lv_obj_t * kb, const char * map[])
{
 ...
 */
static inline void ime_kb_set_ctrl_map(lv_obj_t * kb, const lv_btnmatrix_ctrl_t ctrl_map[])
{
 ...
 */
static inline const char ** ime_kb_get_map_array(const lv_obj_t * kb)
{
