...
static void menu_wifi_test_create();
static void menu_wifi_test_destroy();
static void menu_wifi_test_show();
static void menu_wifi_test_hide();

static menu_page_t menu_wifi_test_page = {
    .onCreate = menu_wifi_test_create,
    .onDestroy = menu_wifi_test_destroy,
    .show = menu_wifi_test_show,
    .hide = menu_wifi_test_hide,
    .back = menuPageBack
};

menu_page_t get_menu_wifi_test_page()
{
    return menu_wifi_test_page;
}

void get_wifi_info_task(lv_task_t * task)
{
	printf("[%s,%d]\n",__FUNCTION__,__LINE__);
	int signal = 0;
	char temBuf[1024] = {0};
	char temBuf2[64] = {0};
    WLAN_AP_INFO_S *wlan_apinfo = CVI_NET_Get_Ap_Info();
	printf("wlan_apinfo:%d \r\n",wlan_apinfo->count);
	for(int i = 0; i< wlan_apinfo->count;i++)
	{
		memset(temBuf,0x00,sizeof(temBuf));
		signal = abs(wlan_apinfo->ap_arr[i].signal_level);
		if(signal >= 30 && signal< 60)
		{
			sprintf(temBuf2,"%s",_("signal level1"));
		}
		else if(signal >= 60 && signal< 90)
		{
			sprintf(temBuf2,"%s",_("signal level2"));
		}
		else
		{
			sprintf(temBuf2,"%s",_("signal level3"));
		}
		snprintf(temBuf,1024,"%s	%s  %s:-%ddBm",wlan_apinfo->ap_arr[i].ssid,temBuf2,_("signal"),signal);

	    lv_label_set_text(show_label[i], temBuf);
	}
	
	//lv_label_set_text(show_label, temBuf);
...
static void _style_init()
{
    if (!styleInited)
    {
		...
		lv_style_init(&disp_wifi_panel_style);
		lv_style_set_bg_opa(&disp_wifi_panel_style, LV_STATE_DEFAULT, LAUNCHER_MAIN_OPACITY);
		lv_style_set_opa_scale(&disp_wifi_panel_style, LV_STATE_DEFAULT, ALTERNATE_UI_OPACITY);
...
static void create_top_panel(lv_obj_t * par)
{
    ...
    lv_label_set_text(page_title, _("WIFI test"));
...
static void menu_wifi_test_create()
{
	...
	struct wlan_cfg cfg;
	sprintf(cfg.ssid,"%s","TP-LINK_5G_JW");
	sprintf(cfg.passwd,"%s","Jw12345678");
	
	//CVI_NET_Set_Wlan_Config(WLAN0_PORT, STATION, &cfg);

	//CVI_NET_Wlan_Init(WLAN0_PORT);

	task = lv_task_create(get_wifi_info_task, 5000, LV_TASK_PRIO_MID, &user_data);
	
    menu_wifi_test_page_obj = lv_cont_create(get_test_mode_page_flag()==1 ? lv_scr_menu_test_mode():lv_scr_menu_factory_mode(), NULL);
    lv_obj_add_style(menu_wifi_test_page_obj, LV_CONT_PART_MAIN, menu_page_test_mode_title_style());
    lv_obj_set_size(menu_wifi_test_page_obj, LV_HOR_RES_MAX, LV_VER_RES_MAX);
	
	create_top_panel(menu_wifi_test_page_obj);
    create_disp_panel(menu_wifi_test_page_obj);
    
	bottom_panel_page_obj = create_bottom_2btn_panel(menu_wifi_test_page_obj, pass_btn_event_cb, error_btn_event_cb);
    ...
    lv_obj_set_hidden(menu_wifi_test_page_obj, true);
}

lv_obj_t * lv_scr_menu_wifi_mode()
{
    return menu_wifi_test_page_obj;
}

static void menu_wifi_test_destroy()
{
    if (menu_wifi_test_page_obj)
    {
        lv_obj_del(menu_wifi_test_page_obj);
        menu_wifi_test_page_obj = NULL;
		if(task != NULL)
		{
			lv_task_del(task);
			task = NULL;
...
static void menu_wifi_test_show()
{
    if (menu_wifi_test_page_obj)
    {
        lv_obj_set_hidden(menu_wifi_test_page_obj, false);
    }
}
