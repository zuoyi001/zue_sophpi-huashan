...
void sdkProcess(std::vector<FaceHandle>& faceHandleArr, const Image& image, const string &writePath){
    FaceRetCode retCode;
    /// create image to draw result
    cv::Mat img;
    if(Format_BGR888 == image.pixel_format){
        img = cv::Mat(image.height, image.width, CV_8UC3, image.vir_addr, image.wstride).clone();
    }
    else if(Format_YUV420P_YU12 == image.pixel_format){
        cv::Mat yuv420(image.height*3/2, image.width, CV_8UC1, image.vir_addr, image.wstride);
        cv::cvtColor(yuv420, img, cv::COLOR_YUV2BGR_IYUV);
    }
    else if(Format_YUV420SP_NV21 == image.pixel_format){
        cv::Mat nv21(image.height*3/2, image.width, CV_8UC1, image.vir_addr, image.wstride);
    ...
    for(size_t i = 0; i < faceHandleArr.size(); ++i){
        ...
        if(RET_OK == retCode){
            printf("[FACEPASS_TEST] getFaceRect: left=%d--top==%d--right=%d--bottom=%d\n", rect.left, rect.top, rect.right, rect.bottom);
        }
        else{
...

        {
            ...
            retCode = getLiveness_bgrir(&image,faceHandleArr[i], &image,faceHandleArr[i], &livenessScore);
        }
        if(RET_OK == retCode){
            printf("[FACEPASS_TEST] getLiveness_bgrir:%f\n", livenessScore);
        }
        else{
            printf("[FACEPASS_TEST] can not detect getLiveness_bgrir!\n");
        }
        ...
        if(RET_OK == retCode){
            ...
            printf("[FACEPASS_TEST] getFaceAttrResult:gender[0]=%f--gender[1]=%f\n", faceAttr.gender[0], faceAttr.gender[1]);
            std::cout<<"respirator=["<<faceAttr.respirator[0]<<", "<<faceAttr.respirator[1]<<", "
                <<faceAttr.respirator[2]<<", "<<faceAttr.respirator[3]<<", "
                <<faceAttr.respirator[4]<<", "<<faceAttr.respirator[5]<<", "
                <<faceAttr.respirator[6]<<"]"<<std::endl;
        }
        else{
        ...
        int featureLength;
        {
            ...
            retCode = extract(&image, faceHandleArr[i], &feature, &featureLength);
        }
        if(RET_OK != retCode){
        ...
        /*FaceOccl faceOccl;
          ...
          if(RET_OK == retCode){
          std::cout<<"leftEyeOccl=["<<faceOccl.leftEye[0]<<", "<<faceOccl.leftEye[1]<<", "
          <<faceOccl.leftEye[2]<<", "<<faceOccl.leftEye[3]<<"]"<<std::endl;
          std::cout<<"rightEyeOccl=["<<faceOccl.rightEye[0]<<", "<<faceOccl.rightEye[1]<<", "
          ...
          std::cout<<"leftCheekOccl=["<<faceOccl.leftCheek[0]<<", "<<faceOccl.leftCheek[1]<<", "
          <<faceOccl.leftCheek[2]<<"]"<<std::endl;
          std::cout<<"rightCheekOccl=["<<faceOccl.rightCheek[0]<<", "<<faceOccl.rightCheek[1]<<", "
          <<faceOccl.rightCheek[2]<<"]"<<std::endl;
          std::cout<<"mouthAndChinOccl=["<<faceOccl.mouthAndChin[0]<<", "<<faceOccl.mouthAndChin[1]<<", "
        ...
        cv::rectangle(img, cv::Point(rect.left, rect.top), cv::Point(rect.right, rect.bottom), _GREEN, 2);
        ...
        cv::putText(img, text, origin, font_face, font_scale, _RED, thickness, 8, 0);
        ...
        origin.x = text_origin_x;
        origin.y += text_stride;
        cv::putText(img, text, origin, font_face, font_scale, MID_GREEN, thickness, 8, 0);
        ...
        origin.x = text_origin_x;
        origin.y += text_stride;
        cv::putText(img, text, origin, font_face, font_scale, Magenta, thickness, 8, 0);
        auto predict = [](float preds[], size_t num) {
            std::vector<std::string> classes = {"no", "sur", "anti", "com", "kitchen", "dif", "un"};
            size_t idx = 0;
            float probability = 0;
        ...
        const auto respirator = predict(faceAttr.respirator, 7);
        sprintf(text, "resp %s:%.2f", respirator.first.c_str(), respirator.second);
        origin.x = text_origin_x;
        origin.y += text_stride;
        const cv::Scalar deeppink(147, 20, 255);
        cv::putText(img, text, origin, font_face, font_scale, deeppink, thickness, 8, 0);
        ...
        /*auto occl_predict = [](float preds[], size_t num) {
          ...
          origin.x = text_origin_x;
          origin.y += text_stride;
          cv::putText(img, text, origin, font_face, font_scale, _YELLOW, thickness, 8, 0);

          const auto leftEye = occl_predict(faceOccl.leftEye, 3);
          sprintf(text, "leftEye %s:%.2f", leftEye.first.c_str(), leftEye.second);
          origin.x = text_origin_x;
          origin.y += text_stride;
          cv::putText(img, text, origin, font_face, font_scale, _YELLOW, thickness, 8, 0);

          const auto rightEye = occl_predict(faceOccl.rightEye, 3);
          sprintf(text, "rightEye %s:%.2f", rightEye.first.c_str(), rightEye.second);
          origin.x = text_origin_x;
          origin.y += text_stride;
          cv::putText(img, text, origin, font_face, font_scale, _YELLOW, thickness, 8, 0);
          ...
          sprintf(text, "leftCheek %s:%.2f", leftCheek.first.c_str(), leftCheek.second);
          origin.x = text_origin_x;
          origin.y += text_stride;
          cv::putText(img, text, origin, font_face, font_scale, _YELLOW, thickness, 8, 0);

          const auto rightCheek = occl_predict(faceOccl.rightCheek, 3);
          sprintf(text, "rightCheek %s:%.2f", rightCheek.first.c_str(), rightCheek.second);
          origin.x = text_origin_x;
          origin.y += text_stride;
          cv::putText(img, text, origin, font_face, font_scale, _YELLOW, thickness, 8, 0);
...
void imageProcess(const string &imagePath, const string &outputFolder){
    ...
    FaceRetCode retCode;
    {
        ...
        retCode = detect((const Image* const)&image, 0.75, MIN_FACE, faceHandleArr.data(), &nCount);
    }
    if(RET_NO_FACE == retCode){
        ...
        const string writePath = cw::Path::join(outputFolder, cw::Path::get_name(imagePath));
...
void yuvProcess(const string &imagePath, const string &outputFolder, int width, int height){
    printf("[FACEPASS_TEST] process image path : %s\n", imagePath.c_str());
    vector<char> data = ImageUtil::readFile(imagePath);
    if(data.empty()){
        printf("[FACEPASS_TEST] load image error!!\n");
        assert(0);
    }
    ...
    int nCount = 128;/*max number of detected faces*/
    std::vector<FaceHandle> faceHandleArr(nCount, nullptr);
    Image image = {static_cast<void*>(data.data()), nullptr, -1, width, height, width, height, Format_YUV420P_YU12};
    FaceRetCode retCode;
    {
        auto sp = cw::ProfilerFactory::get()->make("detect");
        retCode = detect((const Image* const)&image, 0.75, MIN_FACE, faceHandleArr.data(), &nCount);
    ...
    if(RET_NO_FACE == retCode){
        ...
        printf("[FACEPASS_TEST] detect %d faces \n", nCount);
        faceHandleArr.resize(nCount);
        auto writePath = cw::Path::join(outputFolder, cw::Path::get_basename(imagePath)+".png");
        sdkProcess(faceHandleArr, image, writePath);
        for(int i = 0; i < nCount; ++i){
            releaseFace(faceHandleArr[i]);
        }
...
void binaryProcess(const string &imagePath, const string &outputFolder, int width, int height){
    printf("[FACEPASS_TEST] process image path : %s\n", imagePath.c_str());
    vector<char> data = ImageUtil::readFile(imagePath);
    if(data.empty()){
        printf("[FACEPASS_TEST] load image error!!\n");
    ...
    int nCount = 128;/*max number of detected faces*/
    std::vector<FaceHandle> faceHandleArr(nCount, nullptr);
    Image image = {static_cast<void*>(data.data()), nullptr, -1, width, height, width * 3, height, Format_BGR888};
    FaceRetCode retCode;
    {
        auto sp = cw::ProfilerFactory::get()->make("detect");
        retCode = detect((const Image* const)&image, 0.75, MIN_FACE, faceHandleArr.data(), &nCount);
...
static void help(){
    cout << "\nThis program demostrates basic interface facepass sdk\n"
        ...
        "It prints some basic information, and draw face rect and landmark for every face handle.\n"
        "Usage: \n"
        "yuv420 and binary image format, need setting width and height, default 1080*1920\n"
        "./time_test model.yaml -r bgr -i ./image.jpg -o ./output/\n"
        "./time_test model.yaml -r yuv420 -i ./input/ -o ./output/ -w width -h height\n"
        "./time_test model.yaml -r binary -i ./input/ -o ./output/ -w -width -h height\n\n";
}
...
int main(int argc, char *argv[]){
    ...
    parser.add_keyword_arg("-r", "--run_option", "", true, "", "bgr is run bgr image test, yuv420 is run yuv420 image test, binary is run binary format image test");
    parser.add_keyword_arg("-i", "--input_path", "", true, "", "the path or folder for input image");
    ...
    if(runOption == "bgr"){
        if( path.is_dir() ){
            const std::vector<string> images = path.list_file_by_ext_recursive({".jpg", ".bmp", ".png", ".jpeg"});
            printf("[FACEPASS_TEST] %d images in imagePath %s\n", images.size(), imagePath.c_str());
        ...
        if( path.is_dir() ){
            const std::vector<string> images = path.list_file_by_ext_recursive({".yuv"});
            printf("[FACEPASS_TEST] %d images in imagePath %s\n", images.size(), imagePath.c_str());
            for(auto &imageFile : images){
                yuvProcess(imageFile, outputFolder, width, height);
            }
        }
        else{
            yuvProcess(imagePath, outputFolder, width, height);
        }
    }
    else if(runOption == "binary"){
        if( path.is_dir() ){
            const std::vector<string> images = path.list_file_by_ext_recursive({".bmp"});
            printf("[FACEPASS_TEST] %d images in imagePath %s\n", images.size(), imagePath.c_str());
            for(auto &imageFile : images){
