...
int CVI_HAL_TOUCHPAD_Start(int* ps32Fd)
{
    ...
    *ps32Fd = s_s32TOUCHPADfd;
    ...
    {
        sleep(2);
        if (access(HAL_TOUCHAPD_DEV, R_OK) == 0) {
            if (s_s32TOUCHPADfd >= 0) {
            ...
            s_s32TOUCHPADfd = open(HAL_TOUCHAPD_DEV, O_RDONLY);
            if (s_s32TOUCHPADfd < 0) {
                printf("%s:%d: open touchpad failed, fd=%d, filename=%s\n", __func__, __LINE__, s_s32TOUCHPADfd,
                        HAL_TOUCHAPD_DEV);
                return -1;
            } else {
                *ps32Fd = s_s32TOUCHPADfd;
                printf("%s:%d: open touchpad successful, fd=%d, filename=%s\n", __func__, __LINE__,
                        s_s32TOUCHPADfd, HAL_TOUCHAPD_DEV);
            }
        }        
    }
