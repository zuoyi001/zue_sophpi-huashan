...
bool evdev_read(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    ...
    while(read(evdev_fd, &in, sizeof(struct input_event)) > 0) {
		//printf("[in.type:%d in.code:%d in.value:%d %s,%d]\n",in.type,in.code,in.value,__FUNCTION__,__LINE__);
        if(in.type == EV_REL) {
            if(in.code == REL_X)
				#if EVDEV_SWAP_AXES
            ...
            if(in.code == BTN_MOUSE || in.code == BTN_TOUCH) {
				...
				if(LV_INDEV_STATE_PR == evdev_button)
					keyDown = 1;

				//printf("[evdev_button:%x %s,%d]\n",evdev_button,__FUNCTION__,__LINE__);
        } else if(drv->type == LV_INDEV_TYPE_KEYPAD) {
				data->state = (in.value) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
				//printf("[state:%x %s,%d]\n",data->state,__FUNCTION__,__LINE__);
				switch(in.code) {
					case KEY_BACKSPACE:
