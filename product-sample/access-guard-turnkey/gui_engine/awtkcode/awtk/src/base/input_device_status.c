...
//only used on kb
#ifdef CONFIG_INPUT_KB
      widget_t* cur_wdt = window_manager_get_top_window(widget);
      widget_t* src = widget_get_focused_widget(cur_wdt);
      //printf("---- %s \r\n",widget_get_type(src));
      if(evt->key == TK_KEY_ESCAPE){
        window_base_t* base = WINDOW_BASE(cur_wdt);
        ...
        if(base->moving_focus_mode){
          ...
          if(strstr("edit",widget_get_type(src)))
          {
              //printf("ok we are a edit !!!\r\n");
              //改按键为删除
              evt->key = TK_KEY_BACKSPACE;
          }
        }
      }
#endif
