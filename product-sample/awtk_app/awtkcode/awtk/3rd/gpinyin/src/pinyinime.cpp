...
bool im_open_decoder_rom(const char* data) {
  ...
  return matrix_search->init(data, "/userdata.dat");
}
...
unsigned im_delsearch(unsigned pos, bool is_pos_in_splid, bool clear_fixed_this_step) {
  if (NULL == matrix_search) return 0;
...
char16* im_get_candidate(unsigned cand_id, char16* cand_str, unsigned max_len) {
  if (NULL == matrix_search) return NULL;
...
unsigned im_get_predicts(const char16* his_buf, char16 (*&pre_buf)[kMaxPredictSize + 1]) {
